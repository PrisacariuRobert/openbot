...
| General macOS app control | Partial in 0.18 | The Accessibility bridge can list apps, inspect visible controls, focus apps, scroll, and approval-gate clicks/typing/keys; arbitrary pixel and canvas understanding remains unavailable | Grok/Hermes currently broader for visual computer use |
| Large plugin catalog | Partial in 0.18 | Gmail, Drive, Calendar, GitHub, Slack, and Notion are real bounded connectors; there is no third-party marketplace yet | Grok remains broader |
| Always-on hosted cloud computer | Not in 0.18 | Computers are persistent while the local host/Docker are available | Different privacy/availability tradeoff |
