...
| Mobile check-in | Pass for live local-first use | Native SwiftUI companion source, Keychain credential storage, address-only pairing, HttpOnly session handoff, offline/reconnect state, plus the existing 390×844 responsive studio | Native client is now close for foreground use; Grok still has hosted availability, push delivery, and store distribution |
