...
| Studio supervision | Pass | Live Studio combines active work, waiting approvals, failures, progress, browser previews, and direct stop/message controls in one durable view on desktop and iPhone | Stronger local observability and intervention; hosted background availability remains a gap |
...
| Automation reliability | Pass in local-first scope | One real signed delivery created one durable event and linked run, Muse Spark completed the requested artifact, and a repeated delivery ID returned the original receipt without a second run. Schedule, Calendar, GitHub, and generic hooks share the same inbox, replay, rate, failure-pause, and approval path | Stronger duplicate prevention and recovery visibility; unlike Grok's hosted service, local schedules stop when OpenBot sleeps |
...
| Mobile check-in | Pass for live local-first use | Compiled native SwiftUI conversation UI, Keychain credential storage, local and detected Tailscale pairing, bearer-authenticated API, live event updates, teammate targeting, attachment sending, run/approval controls, and a passing live simulator sign-in test | Strong native foreground core without a web-view shell; Grok still has hosted availability, push, richer previews, dedicated voice sessions, and store distribution |
...
| General macOS app control | Partial in 0.19 | The Accessibility bridge can list apps, inspect visible controls, focus apps, scroll, and approval-gate clicks/typing/keys; arbitrary pixel and canvas understanding remains unavailable | Grok/Hermes currently broader for visual computer use |
| Large plugin catalog | Partial in 0.19 | Gmail, Drive, Calendar, GitHub, Slack, and Notion are real bounded connectors; portable browser skills are data-only and there is no third-party executable marketplace yet | Grok remains broader |
| Always-on hosted cloud computer | Not in 0.19 | Computers are persistent while the local host/Docker are available | Different privacy/availability tradeoff |
