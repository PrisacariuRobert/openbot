...
| Large plugin catalog | Partial in 0.26 | Gmail, Drive, Calendar, GitHub, Slack, Notion, Todoist, and Dropbox are real bounded connectors; portable browser skills are data-only and there is no third-party executable marketplace yet | Grok remains broader |
| Always-on computer | Pass with owner setup in 0.26 | The private Linux runner packages HTTPS, restart policy, durable studio/model/browser data, OpenCode, Chromium, and Docker-backed teammate computers. Domain-guided setup, authenticated Home checks, opt-in changed-state alerts, and backup-first updates expose and protect real infrastructure state | Stronger data-location, maintenance, and failure-recovery transparency; Grok remains easier as a managed service |
