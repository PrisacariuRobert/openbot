...
| Background result delivery | Pass in source for web and native | A durable host-owned outbox signs Web Push and APNs, tracks delivery per target, cleans stale endpoints/tokens, bounds payloads, and opens the relevant result or approval. Private mode remains available without the Mac | Better ownership visibility; native delivery still needs owner signing and APNs credentials |
...
| Large plugin catalog | Partial in 0.25 | Gmail, Drive, Calendar, GitHub, Slack, Notion, Todoist, and Dropbox are real bounded connectors; portable browser skills are data-only and there is no third-party executable marketplace yet | Grok remains broader |
| Always-on computer | Pass with owner setup in 0.25 | The private Linux runner packages HTTPS, restart policy, durable studio/model/browser data, OpenCode, Chromium, and Docker-backed teammate computers. Domain-guided setup and authenticated web/iPhone Home checks expose real storage, backup, and tool health | Stronger data-location and infrastructure transparency; Grok remains easier as a managed service |
