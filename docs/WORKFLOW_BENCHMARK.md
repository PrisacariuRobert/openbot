...
| Studio supervision | Pass | Live Studio combines active work, waiting approvals, failures, progress, browser previews, runner health, recovery state, host location, and direct stop/message controls on desktop and iPhone | Stronger ownership visibility and intervention across local or private hosting |
...
| Automation reliability | Pass in owner-controlled scope | Schedule, Calendar, GitHub, and generic hooks share one inbox, replay, rate, failure-pause, and approval path. A single runner lease prevents overlapping processes from dispatching twice; jobs are atomically claimed and an expired job resumes with its checklist intact. Private mode continues on a dedicated host while the Mac is off | Stronger duplicate prevention, restart recovery, explicit location, and owner-visible health; Grok remains easier because Cursor operates its service |
| Background result delivery | Pass in source for web and native | A durable host-owned outbox signs Web Push and APNs, tracks delivery per target, cleans stale endpoints/tokens, bounds payloads, and opens the relevant result, approval, or private-home health view. Private mode remains available without the Mac | Better ownership visibility; native delivery still needs owner signing and APNs credentials |
...
| Workspace research and meeting preparation | Pass in mocked integration; owner sign-in pending | Gmail search/read, Drive search/text export plus reviewed text-file creation, Calendar agenda plus reviewed event/invitation/Meet creation, bounded payloads, separate per-bot read/write grants, and friendly progress are automated; the Apps & Tools panel includes live smoke checks and three useful starters | Closer for core Google Workspace work; existing-file Drive organization and Calendar update/delete remain gaps |
...
| Slack catch-up, event reaction, and approved reply | Pass in mocked/live-shaped integration; owner sign-in pending | Official OAuth/search/conversation/message and signed event shapes, member versus bot authority, bounded results, event/channel filters, per-teammate grants, exact post previews, replay-window and loop protection, scope recovery, and cross-runtime tools are automated | Close for focused Slack knowledge work, with stronger local permission visibility and inspectable wake-ups; broad administration remains a gap |
| Notion knowledge brief, event reaction, and approved append | Pass in mocked/live-shaped integration; owner sign-in pending | Official OAuth/search/page/block/append and signed webhook shapes, selected-page boundary, pagination caps, event/entity filters, per-teammate grants, exact update previews, verification-token flow, rate-limit recovery, and cross-runtime tools are automated | Close for selected knowledge work and additive updates; general database editing remains a gap |
| Mobile check-in | Pass for local or private-host use | Compiled native SwiftUI conversation UI, deliberate editable voice capture, Keychain credential storage, Tailscale/HTTPS connection, live events, teammate targeting, attachment sending, artifact preview/sharing, Share-sheet ingestion, APNs registration/deep links, run/approval controls, runner location/health, recovery state, and manual wake | Private hosting closes powered-off access; Grok still has managed convenience and store distribution |
...
| Proactive connected work | Pass for focused personal and team sources | Calendar, Todoist activity, Dropbox deltas, signed Slack/Notion/GitHub hooks, and signed generic hooks can start bounded work with dedupe, cursors, filters, receipts, and approval-safe tools | OpenBot is inspectable and owner-controlled; Grok remains broader and easier because its event ingress is managed |
| Large plugin catalog | Partial in 0.28 | Gmail, Drive, Calendar, GitHub, Slack, Notion, Todoist, and Dropbox are real bounded connectors; manifest v2 documents event/authenticity admission, while portable browser skills are data-only and there is no third-party executable marketplace yet | Grok remains broader; OpenBot's admission rules are more explicit |
| Always-on computer | Pass with owner setup in 0.27 | The private Linux runner packages HTTPS, restart policy, durable studio/model/browser data, OpenCode, Chromium, and Docker-backed teammate computers. Domain-guided setup, authenticated Home checks, opt-in changed-state alerts, backup-first updates, outside check-ins, and encrypted home transfer expose and protect real infrastructure state | Stronger data-location, maintenance, migration, and failure-recovery transparency; Grok remains easier as a managed service |
| Detect a full private-host outage | Pass with an owner-chosen outside service in 0.27 | OpenBot sends one empty public-HTTPS heartbeat every five minutes; the private URL is encrypted and the external service alerts when pulses stop | More privacy and destination transparency; Grok's managed service needs no owner setup |
| Move an AI home between hosts | Pass through a deliberate encrypted transfer in 0.27 | One authenticated archive carries studio data, vault, model login state, browser profiles, and projects; import stages and validates before backup, swap, health check, and rollback | Stronger owner-controlled portability; not automatic synchronization |
...
- **My private runner breaks while I am away:** opt-in Home checks alert registered devices only when attention changes or a day passes, send one recovery notice, and guided updates keep a fresh backup plus the previous container image until the replacement is healthy.
- **The entire host or network disappears:** an opt-in external heartbeat sends no studio content and lets the owner's chosen service alert after check-ins stop.
- **Moving hosts risks losing credentials or projects:** one passphrase-encrypted whole-home file is authenticated and staged before replacement; import backs up first and restores the prior home if health verification fails.
...
- [Notion — Event delivery and signatures](https://developers.notion.com/reference/webhooks-events-delivery)
