# OpenBot 0.28.0 Launch Readiness

OpenBot 0.28.0 is ready in source as an owner-controlled beta with two honest locations: local on a Mac by default, or always on through an optional private Linux host. Slack and Notion can now wake permissioned teammates from signed provider events on the same durable automation system as schedules, Calendar, Todoist, Dropbox, GitHub, and generic hooks. Provider setup is guided in Apps & Tools, but public HTTPS ingress and live account consent remain owner responsibilities. Native source and simulator builds remain release-ready, while physical-device APNs and App Store distribution still require the owner's Apple team, App Group, signing profile, and `.p8` key. OpenBot closes more of Grok Bot's proactive-work gap for users willing to operate a private host, but it is not a zero-setup managed cloud and still trails in connector breadth, fidelity tools, store distribution, and organization controls.
...
Ship 0.28.0 as an **owner-controlled beta** after the source release gate passes. Slack and Notion event code is release-ready, but do not claim a live provider connection unless the owner's real workspace has completed OAuth, event subscription, and delivery. Local Mac hosting remains the easiest default; reliable public events while it sleeps require the private runner, trusted HTTPS, and provider configuration. Do not describe OpenBot as managed cloud, a connector marketplace, automatic Mac/server synchronization, or maintenance-free hosting. Secure installed web apps have Web Push; the native iPhone source remains simulator-verified rather than claimed App Store distribution. Draft handoff does not yet move unsent attachments, and portable skills cover reviewed browser workflows rather than executable third-party plugins.
