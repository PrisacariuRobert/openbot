...
Updated for OpenBot 0.15.0. This audit compares the locally verified product with the current official Grok Bot product documentation. It separates what works now from what would merely look impressive in a screenshot.

## Current position

OpenBot's strongest difference is owner control: local data, bring-your-own model access, per-teammate app and project permissions, durable approvals, explicit cost limits, isolated code branches, recoverable edits, independent review, and preserved artifact revisions. Version 0.15 adds dependable signed automations with durable receipts, duplicate prevention, visible recovery, and failure pausing. Grok Bot remains broader where a hosted product has an advantage: an always-on cloud computer, native mobile delivery, general computer takeover, a larger connector surface, fidelity-preserving Office work, and organization administration.
...
1. **0.16 — Connector platform:** ship Slack and Notion on the existing per-teammate permission, approval, health, preview, event, and audit contract.
2. **0.17 — Find and organize:** reply context, local full-text search, pins, roster sections, duplication, and private skill import/export.
3. **0.18 — Work anywhere:** optional always-on self-hosted runner, hosted event ingress, push delivery, and then a thin native phone shell without changing local-only mode.
...
OpenBot 0.15.0 is a strong local-first beta, not feature parity with Grok Bot. It is differentiated for private model ownership, granular authority, rich bounded input handling, versioned result cards, coding isolation, change recovery, one-voice teammate consultation, exact-commit review, approval-gated GitHub writes, signed automations with explainable recovery, and portable local skills. “Better” means more trustworthy and owner-controlled first; breadth should be added only when each connector, event source, file processor, and computer action has real permission, recovery, and verification behavior.
