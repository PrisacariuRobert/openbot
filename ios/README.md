...
The iPhone app is a real SwiftUI client for an OpenBot studio that remains hosted by the owner's Mac. Chat, teammate selection, live progress, approvals, connection management, and animated mascots are native controls; the app does not embed the desktop website in a web view. It does not bundle a model runtime or copy the OpenBot database to the phone.
