...
- `npm run test:onboarding`: passed through a separate optional local/API setup, focus refresh with the draft preserved, explicit connection and model selection, and real teammate creation at desktop and 390 px. It started no model job.
...
- With the owner's existing OpenCode Go subscription, `OPENBOT_LIVE_MODEL=opencode-go/muse-spark-1.3-contributor node --import tsx --test src/server/journey-live-smoke.test.ts` passed 2 live tests. A disposable OpenBot teammate conversation completed in 10,774 ms, and the provider reported 13,644 input tokens, 93 output tokens, and a usage value of USD 0.001434276. The synthetic request was to list automations in that conversation. The test verified completion and usage, but did not grade answer correctness, exercise a live browser workflow, or prove an extra charge on the owner's account.
- [Manual cross-platform build-only run](https://github.com/PrisacariuRobert/openbot/actions/runs/35848275237) passed on Apple Silicon Mac, Intel Mac, Windows x64 and Linux x64 at packaging source commit `1d3b036`. It built DMG/ZIP, NSIS EXE, AppImage and DEB on their respective hosts; all four artifact-upload steps were skipped. The repository already held 309 unexpired GitHub Actions artifacts totaling 866.5 MiB, above GitHub Free's 500 MiB included storage. This run requested no additional installer artifact storage. Builds were not clean-installed on independent devices.

This check proves the packaged local client and UI flow, a verified Mac installer image, cross-platform package builds, and one completed OpenCode Go teammate task. It does not prove a successful end-to-end free/local teammate, a useful real-world or live browser task, Windows/Linux clean-device installation, downloadable Windows/Linux artifacts, or signed Mac download.
