...
After the conversation and setup changes were rebased, source commit `59a6f2f` was packaged again with `npm run package:desktop -- --dir`. The packaged Electron app and bundled Node runtime ran `scripts/test-figma-conversation.ts` against an isolated production backend. The first attempt timed out waiting for the keyboard-unpin HTTP response; a clean retry passed pin/unpin, archive/restore, failed-save recovery, the legacy conversation, drafts, search, artifact bytes, and 390/320 px checks. [packaged-conversation-1440x940.png](packaged-conversation-1440x940.png) is the 1440×940 capture from that retry. It uses synthetic data and makes no model or external app call. The timeout remains a test stability concern to watch in CI; it is not evidence of a verified public installer.
