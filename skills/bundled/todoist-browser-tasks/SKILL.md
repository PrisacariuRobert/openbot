...
4. Correct through the task's detail dialog: "Task name" for titles, "Task description" (textbox, then Save) for descriptions, Date and Priority controls for the rest. Save, then read the dialog back. An empty description is a real clear only if the dialog shows it empty.
