...
const tools = [
  ...
  { name: "browser_request_sign_in", description: "Pause the current task and ask the owner to sign in privately on the current browser page. Use for any website needing login, expired sessions, 2FA, CAPTCHA or identity checks. Never include credentials. Resume only after the owner finishes, then verify the page and account.", action: "browser_request_sign_in", properties: {}, required: [] },
  ...
  { name: "routine_create", description: "Create an owner-requested automation. For clock times use schedule {kind:calendar,timeZone:owner IANA zone,time:HH:mm,daysOfWeek:ISO weekdays 1-7}. For once use {kind:once,timeZone,at:ISO timestamp with offset}. Ask if the zone is unknown. intervalMinutes is for elapsed repeats. App events and public webpage watches use triggerConfig; page watches need >=15 minutes.", action: "routine_create", properties: { name: { type: "string" }, prompt: { type: "string" }, schedule: { type: "object", additionalProperties: true }, intervalMinutes: { type: "number", minimum: 5, maximum: 43200 }, triggerType: { type: "string", enum: ["schedule", "calendar", "todoist", "dropbox", "slack", "notion", "webpage"] }, triggerConfig: { type: "object", additionalProperties: true }, enabled: { type: "boolean" } }, required: ["name", "prompt"] },
...
tools.push(
  ...
  { name: "spreadsheet_export", description: "Create a NEW editable .xlsx from visible workspace CSVs without Docker or Excel. Sources preserved, never overwrites, no formulas executed. Link the workbook in your answer. Max 8 sheets, 2 MiB/CSV, 10,000 rows/sheet, 256 columns, 100,000 cells total.", action: "spreadsheet_export", properties: { filename: { type: "string", maxLength: 160, description: "New .xlsx basename, no folders" }, sheets: { type: "array", minItems: 1, maxItems: 8, items: { type: "object", properties: { name: { type: "string", minLength: 1, maxLength: 31 }, csvPath: { type: "string", description: "Visible workspace-relative CSV path" }, numberColumns: { type: "array", maxItems: 256, items: { type: "integer", minimum: 1, maximum: 256 }, description: "1-based numeric columns; header stays text. Default all text. No formatted numbers or more than 15 significant digits." } }, required: ["name", "csvPath"], additionalProperties: false } } }, required: ["filename", "sheets"] },
