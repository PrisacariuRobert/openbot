...
const tools = [
  ...
  { name: "routine_create", description: "Create an OpenBot automation in this conversation. It can run on a schedule or after Calendar, Todoist, Dropbox, Slack, or Notion activity when that connection is ready.", action: "routine_create", properties: { name: { type: "string" }, prompt: { type: "string" }, intervalMinutes: { type: "number", minimum: 5, maximum: 43200 }, triggerType: { type: "string", enum: ["schedule", "calendar", "todoist", "dropbox", "slack", "notion"] }, triggerConfig: { type: "object", additionalProperties: true }, enabled: { type: "boolean" } }, required: ["name", "prompt"] },
