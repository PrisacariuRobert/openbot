...
const tools = [
  ...
  { name: "browser_semantic_act", description: "Click or fill one observed browser control by opaque targetId. Consequential actions pause for exact owner review.", action: "browser_semantic_act", properties: { targetId: { type: "string" }, kind: { type: "string", enum: ["click", "type"] }, value: { type: "string", maxLength: 10000 } }, required: ["targetId", "kind"] },
  { name: "browser_semantic_upload", description: "Select one owner-saved file in an observed file input by opaque targetId. Always pauses for exact owner review before bytes are sent.", action: "browser_semantic_upload", properties: { savedFileId: { type: "string" }, targetId: { type: "string" } }, required: ["savedFileId", "targetId"] },
  { name: "browser_arm_downloads", description: "Arm capture for browser-generated files before clicking a download control. Captures up to six files from this page and its popups for two minutes.", action: "browser_arm_downloads", properties: {}, required: [] },
  { name: "browser_download_results", description: "Check which armed browser downloads were durably saved as conversation attachments. Pending or failed items are not delivered.", action: "browser_download_results", properties: {}, required: [] },
  ...
  { name: "todoist_task_update", description: "Change one Todoist task by its exact id from the task list. Pass an empty description to clear it, or clearDue to drop the due date. Only the listed fields change; always pauses for exact user approval.", action: "todoist_task_update", properties: { taskId: { type: "string" }, content: { type: "string", maxLength: 500 }, description: { type: "string", maxLength: 4000 }, dueString: { type: "string", maxLength: 200 }, clearDue: { type: "boolean" }, priority: { type: "number", minimum: 1, maximum: 4 } }, required: ["taskId"] },
  { name: "todoist_task_complete", description: "Complete one Todoist task by its exact id from the task list. Always pauses for exact user approval; closing is verified by reading the task back.", action: "todoist_task_complete", properties: { taskId: { type: "string" } }, required: ["taskId"] },
  ...
  { name: "task_verify", description: "Record concrete final checks. Add workspace_file evidence for saved text deliverables so OpenBot reopens them and determines whether those checks passed. Pass expectedDigest (SHA-256 of the exact bytes you checked) when you read the file first: a moved file then fails instead of certifying bytes you never saw.", action: "task_verify", properties: { status: { type: "string", enum: ["passed", "partial", "blocked"] }, summary: { type: "string", maxLength: 500 }, checks: { type: "array", minItems: 1, maxItems: 8, items: { type: "object", properties: { label: { type: "string", maxLength: 180 }, passed: { type: "boolean" }, evidence: { type: "object", properties: { kind: { type: "string", enum: ["workspace_file"] }, path: { type: "string", maxLength: 2048 }, minBytes: { type: "number", minimum: 0, maximum: 500000 }, contains: { type: "array", maxItems: 8, items: { type: "string", minLength: 1, maxLength: 200 } }, expectedDigest: { type: "string", pattern: "^[a-fA-F0-9]{64}$" } }, required: ["kind", "path"], additionalProperties: false } }, required: ["label", "passed"], additionalProperties: false } } }, required: ["status", "summary", "checks"] },
  ...
  { name: "routine_list", description: "List this conversation's automations with ids, schedules and on/off state. Pass one exact routineId to read just that routine's instructions and revision before or after an edit; broad lists omit instructions. Ask when several could match the owner's meaning.", action: "routine_list", properties: { query: { type: "string", maxLength: 200 }, routineId: { type: "string" } }, required: [] },
  { name: "routine_update", description: "Change one listed automation by its exact id: rename it, rewrite its instructions, or change its repeat time. Pass the revision routine_list showed; a changed revision is a conflict, never a silent overwrite. Enabling or broadening a routine always pauses for the owner's exact review.", action: "routine_update", properties: { routineId: { type: "string" }, expectedRevision: { type: "number" }, name: { type: "string", maxLength: 80 }, prompt: { type: "string", maxLength: 10000 }, intervalMinutes: { type: "number", minimum: 5, maximum: 43200 }, schedule: { type: "object", additionalProperties: true } }, required: ["routineId"] },
  { name: "routine_pause", description: "Pause one listed automation by its exact id so it stops future runs. The running instance, if any, is a separate stop decision. Pausing never needs a review; resuming does.", action: "routine_pause", properties: { routineId: { type: "string" }, expectedRevision: { type: "number" } }, required: ["routineId"] },
  { name: "routine_resume", description: "Resume one listed paused automation by its exact id. Grants, trigger setup and the next run time are rechecked, and the resume always pauses for the owner's exact review. A changed revision is a conflict, never a silent overwrite.", action: "routine_resume", properties: { routineId: { type: "string" }, expectedRevision: { type: "number" } }, required: ["routineId"] },
  { name: "routine_delete", description: "Delete one listed automation by its exact id so it never runs again. Past conversation results stay available and the deletion is recorded. Deletion always pauses for the owner's exact review; a changed revision is a conflict, never a silent delete.", action: "routine_delete", properties: { routineId: { type: "string" }, expectedRevision: { type: "number" } }, required: ["routineId"] },
  ...
  { name: "handoff", description: "Privately delegate a focused deliverable, then wait and synthesize the teammate's result into your answer.", action: "handoff", properties: { botId: { type: "string" }, task: { type: "string" }, artifacts: { type: "array", maxItems: 6, items: { type: "object", properties: { artifactId: { type: "string" }, path: { type: "string" }, access: { type: "string", enum: ["read"] } } } }, dedupeKey: { type: "string" } }, required: ["botId", "task", "dedupeKey"] },
  { name: "message_teammate", description: "Privately send a focused question, update, or finding. A requested reply pauses your final answer until the result is ready.", action: "message_teammate", properties: { botId: { type: "string" }, message: { type: "string" }, kind: { type: "string", enum: ["message", "question", "finding"] }, expectsReply: { type: "boolean" }, artifacts: { type: "array", maxItems: 6, items: { type: "object", properties: { artifactId: { type: "string" }, path: { type: "string" }, access: { type: "string", enum: ["read"] } } } }, dedupeKey: { type: "string" }, replyToId: { type: "string" } }, required: ["botId", "message", "kind", "expectsReply", "dedupeKey"] },
