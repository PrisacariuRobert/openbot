...
export function connectedAppsText(db: OpenBotDatabase, bot: Bot) {
  ...
  const gmailText = googleCapability.gmail.read && gmail?.canRead && !unavailable.has("gmail")
    ? `- Gmail search and reading are available now.${gmail.canSend && googleCapability.gmail.write ? " You may prepare an email, but gmail_send will always pause for the user to approve the exact recipient and subject." : " Sending is turned off or needs a Google reconnect."}`
...
- Do not claim Mail or Calendar is unavailable just because Google is disconnected. When Mac access is enabled, work_collect can read the built-in Mail/Calendar apps as a read-only fallback. Prefer this bounded source tool before generic app controls. It still requires macOS Automation consent and an available Mac. Name the actual source, disclose partial sync/recurrence coverage, and never present Apple Mail messages as complete Gmail threads or infer that a reply is owed. For other installed apps, use mac_apps_list and mac_app_inspect only when Mac access is enabled; do not invent a connection, permission, or successful action.
- For an editable spreadsheet, save your data as CSV in this workspace, then use spreadsheet_export to create a NEW .xlsx workbook. No computer, Docker, Excel, or browser is needed. Preserve original sources; create separate analysis CSVs. Cells stay literal text unless you explicitly name 1-based numberColumns (the header stays text). Never put formulas in numeric columns. Include source IDs and policy references when reconciling data, verify calculations against the full source rather than a clipped preview, and link the workbook in your final answer. Export receipts verify bytes and counts, not your reasoning or Excel's visual layout.
...
`;
  ...
  const agentPermission = reportOnly
    ...
    : { "*": "allow", ...Object.fromEntries(Object.entries(availableTools).map(([name, available]) => [name, available ? "allow" : "deny"])), external_directory: "deny" };
  ...
  writeFileSync(path.join(toolsDir, "spreadsheet_export.ts"), toolFile("spreadsheet_export", "Create a new editable .xlsx from visible workspace CSV files without Docker or Excel. Preserves sources, never overwrites. No formulas executed. Link the returned workbook in your answer. Max 8 sheets, 2 MiB/CSV, 10,000 rows/sheet, 256 columns, 100,000 cells total.", `filename: tool.schema.string().max(160).describe("New .xlsx basename, no folders"), sheets: tool.schema.array(tool.schema.object({ name: tool.schema.string().min(1).max(31), csvPath: tool.schema.string().describe("Visible workspace-relative CSV path"), numberColumns: tool.schema.array(tool.schema.number().int().min(1).max(256)).max(256).optional().describe("1-based numeric columns; header stays text. Default all text. No formatted numbers or more than 15 significant digits.") })).min(1).max(8)`, "spreadsheet_export"), "utf8");
  ...
  writeFileSync(path.join(toolsDir, "browser_request_sign_in.ts"), toolFile("browser_request_sign_in", "Pause this task and ask the owner to sign in privately on the current page. Use for any website needing login, expired sessions, 2FA, CAPTCHA or identity checks. Never include credentials. After the owner continues, verify the page and account before resuming work.", ``, "browser_request_sign_in"), "utf8");
