...
`, "utf8");
  ...
  writeFileSync(path.join(toolsDir, "routine_list.ts"), toolFile("routine_list", "List the automations of this conversation with their ids, names, schedules and on/off state. Use the ids to refer to one exact routine in follow-ups; when several could match what the owner means, ask which one instead of guessing. Only routines in this conversation are visible.", `query: tool.schema.string().max(200).optional().describe("Optional case-insensitive name filter")`, "routine_list"), "utf8");
