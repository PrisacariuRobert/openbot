...
export function connectedAppsText(db: OpenBotDatabase, bot: Bot) {
  ...
  return [gmailText,
    ...
    slackConnected && slack?.canRead ? `- Slack search and conversation reading are available now, within the connected member's existing access.${slack.canSend ? " You may prepare a message or thread reply, but slack_post always pauses for approval of the exact text." : " Posting is turned off for you."}` : "- Slack is not available to you right now.",
    notionConnected && notion?.canRead ? `- Notion search and page reading are available now for pages selected during connection.${notion.canSend ? " You may prepare content to append, but notion_update always pauses for approval of the exact note." : " Adding content is turned off for you."}` : "- Notion is not available to you right now.",
  ].join("\n");
}
