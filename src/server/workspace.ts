...
- For code inside a shared user project, use code_projects to identify the approved project, then code_list/code_search/code_read before editing. Start a separate code_branch before work, use code_replace for focused changes or code_write for complete files, inspect code_diff, run code_run checks, and use code_commit with only the exact changed paths. Call task_verify before code_publish_pr; publishing always pauses for the user's approval. Never use Mac file tools to bypass project permissions.
...
- Use message_teammate quietly to ask a focused question or share a useful finding. Request a reply only when it is genuinely needed.
- Handoff only when another teammate is clearly better suited, and include a specific deliverable.
...
`;
  ...
  writeFileSync(path.join(toolsDir, "code_branch.ts"), toolFile("code_branch", "Create and switch to a separate Git branch in a clean shared project.", `projectId: tool.schema.string(), name: tool.schema.string().max(120)`, "code_branch"), "utf8");
  ...
  writeFileSync(path.join(toolsDir, "code_publish_pr.ts"), toolFile("code_publish_pr", "Publish the tested branch as a GitHub pull request. Requires passed task verification and always pauses for user approval.", `projectId: tool.schema.string(), title: tool.schema.string().max(160), body: tool.schema.string().max(10000), base: tool.schema.string().max(120).optional(), draft: tool.schema.boolean().optional()`, "code_publish_pr"), "utf8");
  ...
  writeFileSync(path.join(toolsDir, "handoff.ts"), toolFile("handoff", "Hand a focused follow-up task to another OpenBot teammate.", `botId: tool.schema.string(), task: tool.schema.string(), dedupeKey: tool.schema.string()`, "handoff"), "utf8");
  writeFileSync(path.join(toolsDir, "message_teammate.ts"), toolFile("message_teammate", "Send a question, update, or finding to another teammate. Ask for a reply only when the current task truly needs one.", `botId: tool.schema.string(), message: tool.schema.string(), kind: tool.schema.enum(["message", "question", "finding"]), expectsReply: tool.schema.boolean(), dedupeKey: tool.schema.string(), replyToId: tool.schema.string().optional()`, "message_teammate"), "utf8");
