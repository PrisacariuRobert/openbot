...
export function browserAccessText(db: OpenBotDatabase, bot: Bot) {
  ...
  // Services in the same unavailable state share one line instead of
  // repeating the same sentence per service in every message.
  const unavailable = new Map<string, string[]>();
  for (const route of status.services) {
    if (route.connectorState === "ready" || route.browserState === "read-denied" || route.browserState === "available-unverified") continue;
    unavailable.set(route.connectorState, [...(unavailable.get(route.connectorState) || []), route.label]);
  }
  const grouped = [
    ...lines.filter((line) => !/ and your browser is off\. /.test(line)),
    ...[...unavailable].map(([state, labels]) => `- ${labels.join(", ")}: the structured connector is ${state} and your browser is off. Ask the owner to share a connector or enable browser access; do not enable either yourself.`),
  ];
  if (!bot.browserEnabled) return grouped.join("\n");
  return `${grouped.join("\n")}\n\nUse only your own persistent browser profile. Never extract cookies, tokens, passwords, passkeys, or another browser/profile's credentials, and never copy a login to another teammate. For login, expired sessions, 2FA, CAPTCHA or identity verification on any website (not just connected apps), call browser_request_sign_in on the current page — but ONLY with cited evidence: the page URL as observedUrl and the exact wall text as observedText. A loading or half-rendered page is not signed-out: wait for it to finish, snapshot again, and only request a handoff when a login form, an account chooser with no logged-in account, or an explicit session-expired page persists. A false sign-in alarm costs the owner real time, so an uncited request is a defect. It saves the task, notifies the owner and pauses browser work for private owner takeover. Do not merely tell them to connect an API, claim there is no access, or handle credentials yourself; never request secrets in chat or work around a site's check. Once the owner finishes, inspect the page and account before continuing. A successful sign-in is not permanent access or permission for every task. External writes still require their normal approval and must not bypass a denied action. Name browser-derived sources honestly, with their URLs; do not pretend they came from a connector or an exhaustive inbox/calendar snapshot.`;
}
