...
import { ComputerManager } from "./runtime.js";
...
test("host mediation denies sibling/ambient access and allows own mediated file work", { timeout: 90_000 }, async () => {
  ...
  const beta = db.createBot({ name: "Beta", emoji: "B", color: "#aa3344", role: "Reviewer", instructions: "Review results.", providerInstanceId: "local-opencode", model: "opencode/fixture", computerEnabled: false });
  ...
  assert.equal(obs!.ambientBash!.status, 403, "ambient shell is refused when the teammate computer is off");
...
async function main() {
  ...
  const call = (action, args) => fetch(endpoint + '/api/internal/tools', { method: 'POST', headers, body: JSON.stringify({ botId: process.env.OPENBOT_BOT_ID, runId, action, args }) }).then(async (r) => ({ status: r.status, body: await r.text() }));
  const obs = {};
  obs.proofOfExecution = await call('bash', { command: 'echo CONTAINED_OK' });
  obs.siblingAbsoluteCat = await call('bash', { command: 'cat ' + alphaCanary });
  fs.writeFileSync(record, JSON.stringify(obs));
  console.log(JSON.stringify({ type: 'text', text: 'Probe complete.' }));
}
main().catch((error) => { console.error(error.message); process.exitCode = 1; });
...
/** A computer-enabled teammate's shell runs inside its own container, which
 * bind-mounts only that teammate's workspace. An absolute host path to a
 * sibling workspace must resolve to nothing useful: the canary bytes must
 * never come back. Found because Linux CI (Docker available) executed the
...
test("computer-on shell cannot leak a sibling workspace through absolute paths", { timeout: 300_000 }, async () => {
  ...
  const bin = path.join(root, "bin"); mkdirSync(bin);
  writeFileSync(path.join(bin, "opencode"), ESCAPE_RUNTIME, { mode: 0o700 });
  const alpha = db.createBot({ name: "Alpha", emoji: "A", color: "#3344aa", role: "Analyst", instructions: "Store results.", providerInstanceId: "local-opencode", model: "opencode/fixture", computerEnabled: true });
  const beta = db.createBot({ name: "Beta", emoji: "B", color: "#aa3344", role: "Reviewer", instructions: "Review results.", providerInstanceId: "local-opencode", model: "opencode/fixture", computerEnabled: true });
  const canary = "OB-COMPUTER-ESCAPE-HOSTBOUNDARY";
  mkdirSync(path.join(db.workspacesDir, alpha.id), { recursive: true });
  mkdirSync(path.join(db.workspacesDir, beta.id), { recursive: true });
  const canaryPath = path.join(db.workspacesDir, alpha.id, "canary.txt");
  writeFileSync(canaryPath, canary + "\n");
  ...
  try {
    ...
    assert.ok(ready, log || "server did not start");
    const sent = await fetch(base + "/api/messages", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ threadId: beta.threadId, body: "Check whether you can see the sibling canary.", targetBotIds: [beta.id] }) });
    assert.equal(sent.status, 202, await sent.clone().text());
    const { runs } = await sent.json() as { runs: Array<{ id: string }> };
    const runId = runs[0]!.id;
