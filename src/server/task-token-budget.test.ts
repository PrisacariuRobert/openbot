...
// The scenario: a task past a 100k per-task limit (defaults are higher now).
const LIMITS = { ...DEFAULT_EXECUTION_LIMITS, maxTokens: 100_000, maxJobTokens: 100_000 };
...
test('one durable task-only grant resumes the same job without resetting usage, steps, session or progress', () => {
  ...
  try {
    ...
    const review = f.db.taskTokenReview(a.id, LIMITS)!;
    ...
    assert.equal(f.db.decideTaskTokens(a.id, 'approved', LIMITS)?.status, 'approved');
    assert.equal(f.db.decideTaskTokens(a.id, 'approved', LIMITS), null, 'No duplicate grant');
...
test('late usage changes the reviewed total, and another ceiling needs a new approval', () => {
  ...
  try {
    ...
    assert.equal(f.db.taskTokenReview(a.id, LIMITS)!.newJobLimit, 173_800);
    f.db.decideTaskTokens(a.id, 'approved', LIMITS);
    ...
    assert.equal(f.db.taskTokenReview(a.id, LIMITS), null);
    assert.equal(f.db.decideTaskTokens(again.id, 'denied', LIMITS)!.status, 'denied');
...
test('weekly limits still block spending and declining stops every paused consultant, not unrelated work', () => {
  ...
  try {
    ...
    assert.match(f.db.taskTokenReview(a.id, LIMITS)!.limitation!, /weekly/);
    assert.equal(f.db.decideTaskTokens(a.id, 'approved', LIMITS), null);
...
test('an action already in flight must finish before a token grant and cannot be dispatched twice', () => {
  ...
  try {
    ...
    const a = f.db.pauseForTaskTokens(f.run.id)!;
    assert.match(f.db.taskTokenReview(a.id, LIMITS)!.limitation!, /already-approved action/);
    assert.equal(f.db.decideTaskTokens(a.id, 'approved', LIMITS), null);
    f.db.completeApprovedAction(external.id, 'Fixture accepted once');
    assert.equal(f.db.taskTokenReview(a.id, LIMITS)!.limitation, null);
    assert.equal(f.db.decideTaskTokens(a.id, 'approved', LIMITS)?.status, 'approved');
