...
export const CONNECTOR_MANIFESTS: readonly ConnectorManifest[] = [
  {
    schemaVersion: 1, connectorId: "google-workspace", service: "gmail", name: "Gmail",
    description: "Search and read mail, then send only after your approval.", auth: "oauth",
    readCapability: "Search and read messages", writeCapability: "Send plain-text email", writeRequiresApproval: true,
  ...
  {
    schemaVersion: 1, connectorId: "google-workspace", service: "google-drive", name: "Google Drive",
    description: "Find documents and bring current project context into a conversation.", auth: "oauth",
    readCapability: "Search and read supported documents", writeCapability: null, writeRequiresApproval: false,
  ...
  {
    schemaVersion: 1, connectorId: "google-workspace", service: "google-calendar", name: "Google Calendar",
    description: "Check upcoming events and use them as automation triggers.", auth: "oauth",
    readCapability: "Read the primary calendar", writeCapability: null, writeRequiresApproval: false,
  ...
  {
    schemaVersion: 1, connectorId: "github-cli", service: "github", name: "GitHub",
    description: "Follow notifications and issues, then create issues only after approval.", auth: "local_cli",
    readCapability: "Read notifications and search issues", writeCapability: "Create issues", writeRequiresApproval: true,
  ...
  {
    schemaVersion: 1, connectorId: "slack", service: "slack", name: "Slack",
    description: "Search conversations, read context, and post a reviewed reply.", auth: "oauth",
    readCapability: "Search messages and read conversation context", writeCapability: "Post a message or thread reply", writeRequiresApproval: true,
  ...
  {
    schemaVersion: 1, connectorId: "notion", service: "notion", name: "Notion",
...
export function validateConnectorManifests(manifests: readonly ConnectorManifest[] = CONNECTOR_MANIFESTS): string[] {
  ...
  for (const manifest of manifests) {
    if (manifest.schemaVersion !== 1) errors.push(`${manifest.service} uses an unsupported manifest version.`);
