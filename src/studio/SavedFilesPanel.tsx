...
export function SavedFilesPanel({ bot }: { bot: Bot }) {
  ...
  >
    ...
    {loading ? <div className="saved-files-state"><LoaderCircle className="spinner" size={18} /> Loading saved files…</div> : files.length === 0 && !error ? <div className="saved-files-dropzone" role="button" tabIndex={0} aria-label={`Add files for ${bot.name}`} onClick={() => inputRef.current?.click()} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); inputRef.current?.click(); } }}><FileIcon size={22} /><p><strong>Drop files here</strong><span>A CV or reference {bot.name} should reuse.</span></p></div> : files.length > 0 ? <div className="saved-files-list">{files.map((file) => <div className="saved-file-row" key={file.id} title={`Saved ${file.savedAt} · SHA-256 ${file.sha256}`}><FileIcon size={18} /><span className="saved-file-meta"><strong>{file.name}</strong><small>{formatSize(file.size)} · Saved {formatSavedAt(file.savedAt)} · {file.sha256.slice(0, 8)}</small></span><a href={file.url} target="_blank" rel="noreferrer">Open</a>{confirmId === file.id ? <span className="saved-file-confirm" role="group" aria-label={`Remove ${file.name} from library`}><span>Remove from library?</span><small>Removes it from this library, but does not erase existing messages or the original upload.</small><button type="button" disabled={busy} onClick={() => setConfirmId(null)}>Cancel</button><button type="button" disabled={busy} onClick={() => void remove(file.id)}><Trash2 size={14} /> Remove</button></span> : <button type="button" className="saved-file-remove" disabled={busy} onClick={() => setConfirmId(file.id)}>Remove</button>}</div>)}</div> : null}
  </section>;
}
