...
export function DocumentPane({ file, files, modal, canRevise, onClose, onRevise }: {
  ...
  const content = <>
    ...
    <div className="document-reader"><article className="rendered-markdown">{selected.previewText ? <ReactMarkdown components={{ table: ({ children }) => <div className="markdown-table"><table>{children}</table></div> }} remarkPlugins={[remarkGfm]} skipHtml disallowedElements={["img"]}>{selected.previewText}</ReactMarkdown> : selected.kind === "image" && selected.previewUrl ? <img src={selected.previewUrl} alt={selected.name}/> : <p>A text preview is unavailable. Open the original file below.</p>}{selected.processingStatus === "partial" && <p className="document-preview-note">Partial preview. The original file contains the full result.</p>}</article></div>
