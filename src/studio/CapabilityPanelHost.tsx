...
import { BotPanel, ArtifactsPanel, CodeProjectsPanel, ComputerPanel, ConnectorPanel, ControlPanel, FilesPanel, LiveStudioPanel, RemotePanel, RoutinesPanel, SearchPanel, TeachPanel } from "../CapabilityPanels";
...
export function CapabilityPanelHost({ panel, state, threadId, onOpen, onThread, onChange }: {
  ...
  return <div className={`capabilities capability-${panel}`}>
    ...
    {panel === "teach" && <><ExtensionsPanel bots={state.bots} skillsOnly />{bot && <details className="capability-disclosure"><summary>Teach and manage a browser workflow</summary><TeachPanel key={bot.id} bot={bot} bots={state.bots} onBotChange={setChosenBot} onNotice={setNotice} onUse={async (workflow) => {
    ...
    {panel === "live" && <><LiveStudioPanel state={state} onOpenThread={onThread} onReview={setReviewRunId} onCancel={async (id) => { await change(`/api/runs/${id}/cancel`); }}
      ...
      {reviewRun && <section className="capability-review"><h3>Review request</h3><RunControls run={reviewRun} approval={state.approvals.find((item) => item.runId === reviewRun.id)} onChange={onChange} /></section>}</>}
