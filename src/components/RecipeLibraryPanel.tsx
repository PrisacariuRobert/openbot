...
export function RecipeLibraryPanel({ bots, request, onStart }: { bots: Bot[]; request: <T>(url: string, init?: RequestInit) => Promise<T>; onStart: (prompt: string, kind?: "morning" | "inbox" | "meeting" | "weekly", botId?: string) => Promise<void> }) {
  ...
  return <details className="recipe-library">
    ...
    <div className="recipe-body">
      ...
      {example && <details className="recipe-result" open><summary>Example result · no model used</summary><ul>{example.checks.map((check) => <li key={check}>{check}</li>)}</ul><div className="recipe-markdown"><ReactMarkdown remarkPlugins={[remarkGfm]} skipHtml>{example.markdown}</ReactMarkdown></div></details>}
