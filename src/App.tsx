...
import {
  ArrowLeft, ArrowUp, AtSign, Bell, CalendarDays, Check, ChevronDown, CircleAlert, Clock3, Code2, Coins, Copy, Cpu, Download, ExternalLink, Eye, File, FileArchive, FileAudio, FileImage, FileSpreadsheet, FileText, FileVideo,
  ...
  Presentation, RotateCcw, Terminal, Trash2, Users, WandSparkles, Webhook, Wifi, WifiOff, Workflow, X,
} from "lucide-react";
import type {
...
const MASCOT_KINDS: MascotKind[] = ["orbit", "nova", "blob", "sprout", "pebble", "sunny"];
const MASCOT_COLORS = ["#ee8b46", "#6757d9", "#ef6a8a", "#27a67a", "#3187dc", "#d49b16"];
...
function Sidebar({ state, provider, connectors, activeThreadId, onSelectThread, onCreateBot, onOpenPanel, open, onClose }: {
  ...
  const connectedApps = connectors?.catalog.filter((item) => item.connected).length || 0;
  return <>
    ...
    <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
      ...
      <nav className="sidebar-scroll" aria-label="Conversations">
        ...
        <div className="bot-list">{bots.map((bot) => <button key={bot.id} className={`nav-row ${activeThreadId === bot.threadId ? "active" : ""}`} onClick={() => onSelectThread(bot.threadId)}>
          ...
          <div className="nav-copy"><strong>{bot.name}</strong><span>{bot.status === "working" ? "Working on it…" : bot.status === "waiting" ? "Waiting…" : bot.status === "failed" ? "Needs a hand" : bot.role}</span></div>
      ...
      <div className="sidebar-footer">
        ...
        <button className="apps-pill" onClick={() => onOpenPanel("connectors")}><span className="apps-icon"><ConnectorIcon id={connectors?.slack.connected ? "slack" : connectors?.notion.connected ? "notion" : "gmail"} /></span><span><strong>Apps & tools</strong><small>{connectedApps ? `${connectedApps} app${connectedApps === 1 ? "" : "s"} ready` : "Connect your work"}</small></span><i className={connectedApps ? "ready" : ""} /></button>
...
function ConversationHeader({ threadTitle, activeBot, roomBots, onMenu, onOpenPanel, connection = "online" }: {
  ...
  return <header className="conversation-header">
    ...
    <div className="header-actions">
      ...
      <button className="icon-button" onClick={() => onOpenPanel("routines")} aria-label="Open automations"><Workflow size={19} /></button>
...
function MessageText({ body, attachments = [] }: { body: string; attachments?: Attachment[] }) {
  return <div className="message-content"><Suspense fallback={<p>{body}</p>}><MarkdownMessage body={body} attachments={attachments} /></Suspense></div>;
}
...
function RunCard({ run, onApprove, onCancel }: { run: Run; onApprove: (id: string) => void; onCancel: (id: string) => void }) {
  ...
  const waiting = run.status === "awaiting_approval", consulting = run.status === "waiting_for_teammate", failed = run.status === "failed";
  const status: BotStatus = waiting || consulting ? "waiting" : failed ? "failed" : "working";
  ...
  return <article className={`run-card ${waiting ? "run-approval" : ""} ${consulting ? "run-consulting" : ""} ${failed ? "run-failed" : ""}`}>
    <div className="run-topline"><Mascot bot={bot} size="small" /><div className="run-title"><strong>{waiting ? `${run.botName} needs your okay` : consulting ? `${run.botName} is consulting the team` : failed ? `${run.botName} hit a snag` : run.task.goal}</strong><span>{failed ? run.error : consulting ? "Bringing their ideas into one answer" : `${run.botName} · ${taskStageLabels[run.task.stage]}`}</span></div>{(run.status === "running" || consulting) && <LoaderCircle className="spinner" size={18} />}</div>
    {!failed && run.task.tracked && <><div className="task-progress-head"><span>{waiting ? "Paused safely" : consulting ? "Comparing ideas" : taskStageLabels[run.task.stage]}</span>{run.task.steps.length > 0 && <b>{completedSteps} of {run.task.steps.length}</b>}</div><div className="task-progress-track"><i style={{ width: `${waiting || consulting ? progress : Math.max(progress, run.status === "running" ? 7 : 0)}%` }} /></div><div className="task-step-list">{run.task.steps.map((step) => <div key={step.id} className={`task-step task-step-${step.status}`}><span><TaskStepMark status={step.status} /></span><p>{step.title}{step.detail && <small>{step.detail}</small>}</p></div>)}</div></>}
    ...
    {(run.status === "running" || consulting) && <button className="stop-button" onClick={() => onCancel(run.id)}><Square size={10} fill="currentColor" /> Stop</button>}
  </article>;
}
...
function Composer({ threadId, sharedDraft, bots, apps = [], skills = [], isRoom, selectedBotIds, setSelectedBotIds, workingBotIds = [], onSend, sending, onNotice }: {
  ...
  const appAliases: Partial<Record<ConnectorStatus["catalog"][number]["id"], string>> = { gmail: "gmail", "google-drive": "drive", "google-calendar": "calendar", github: "github", slack: "slack", notion: "notion" };
  ...
  const insertSkill = (skill: TaughtWorkflow) => {
    ...
    const match = before.match(/(?:^|\s)\/([a-z0-9-]*)$/i), start = match ? cursor - (match[1]?.length || 0) - 1 : cursor;
    const prefix = start > 0 && !/\s$/.test(body.slice(0, start)) ? " " : "";
    setBody(`${body.slice(0, start)}${prefix}/${skill.skillSlug} ${after}`); setSkillQuery(null); setMentionQuery(null); setRoutingOpen(false); setSelectedBotIds([skill.botId]);
  ...
  const submit = async () => {
    ...
    setBody(""); setFiles([]); setMentionQuery(null); setSkillQuery(null); setRoutingOpen(false);
  ...
  return <div className="composer-wrap" data-testid="composer-wrap">
    ...
    {skillQuery !== null && <div className="routing-popover skill-popover" role="listbox" aria-label="Choose a learned skill">
      {skillChoices.length ? skillChoices.map((skill) => { const owner = bots.find((bot) => bot.id === skill.botId); return <button key={skill.id} onClick={() => insertSkill(skill)}><span className="route-symbol"><WandSparkles size={16} /></span><span><strong>/{skill.skillSlug}</strong><small>{skill.name} · {owner?.name || skill.botName}</small></span><ArrowUp size={13} /></button>; }) : <div className="skill-empty"><WandSparkles size={17} /><span><strong>No matching skill</strong><small>Teach a teammate once, then reuse it here.</small></span></div>}
    </div>}
...
function ControlPanel({ state, onNotify, onOpenProvider, onOpenRemote, onOpenConnectors, onOpenProjects, onSetMacAccess }: { state: AppState; onNotify: () => void; onOpenProvider: () => void; onOpenRemote: () => void; onOpenConnectors: () => void; onOpenProjects: () => void; onSetMacAccess: (enabled: boolean) => Promise<void> }) {
  ...
  return <div className="control-panel">
    ...
    <section><div className="panel-section-heading"><div><h3>Right now</h3><p>Real activity from every teammate</p></div></div>{active.length ? <div className="now-list">{active.map((bot) => <div key={bot.id}><Mascot bot={bot} size="medium" /><span><strong>{bot.name}</strong><small>{bot.status === "waiting" ? state.approvals.some((approval) => approval.botId === bot.id) ? "Waiting for your okay" : "Consulting the team" : "Working in the background"}</small></span><i className={`now-dot now-${bot.status}`} /></div>)}</div> : <div className="calm-card"><RoomCluster bots={state.bots} large /><span><strong>Ready when you are</strong><small>Start in any conversation. Up to three teammates can work at once.</small></span></div>}</section>
...
function CodeProjectsPanel({ bots, onNotice }: { bots: Bot[]; onNotice: (message: string) => void }) {
  ...
  const openReview = async (project: CodeProject, workspace?: CodeTaskWorkspace) => {
    const key = workspace?.runId || "base", currentKey = review?.workspace?.runId || "base";
    if (review?.projectId === project.id && currentKey === key) { setReview(null); return; }
    setBusy(`review:${project.id}:${key}`); setError("");
    try { setReview(await api<CodeProjectReview>(`/api/code-projects/${project.id}/review${workspace ? `?runId=${encodeURIComponent(workspace.runId)}` : ""}`)); }
    catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); }
    finally { setBusy(null); }
  };
  const restore = async (edit: CodeProjectEdit) => {
    ...
    try { await api(`/api/code-project-edits/${edit.id}/restore`, { method: "POST" }); await refresh(); if (review?.projectId === edit.projectId) setReview(await api<CodeProjectReview>(`/api/code-projects/${edit.projectId}/review${review.workspace ? `?runId=${encodeURIComponent(review.workspace.runId)}` : ""}`)); onNotice(`${edit.path} restored safely`); }
    catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); }
    finally { setBusy(null); }
  };
  return <div className="code-projects-panel">
    ...
    {data?.projects.length ? <section><div className="panel-section-heading"><div><h3>Connected projects</h3><p>Give every teammate exactly the access their role needs</p></div><span className="bounded-badge">{data.projects.length} connected</span></div><div className="code-project-list">{data.projects.map((project) => <article key={project.id}>
      ...
      {data.workspaces.filter((workspace) => workspace.projectId === project.id && workspace.status !== "archived").length > 0 && <div className="project-workspaces"><div className="project-workspaces-title"><span><GitBranch size={13} /> Agent workspaces</span><small>Your main folder stays untouched</small></div>{data.workspaces.filter((workspace) => workspace.projectId === project.id && workspace.status !== "archived").map((workspace) => { const taskBot = bots.find((bot) => bot.id === workspace.botId), codeReview = data.reviews.find((item) => item.sourceRunId === workspace.runId); const label = workspace.status === "published" ? "Published" : codeReview?.verdict === "approved" ? `${codeReview.reviewerBotName} approved` : codeReview?.verdict === "changes_requested" ? "Changes requested" : "In progress"; const state = workspace.status === "published" ? "published" : codeReview?.verdict === "approved" ? "approved" : codeReview?.verdict === "changes_requested" ? "changes" : "active"; return <button key={workspace.runId} onClick={() => void openReview(project, workspace)} disabled={busy === `review:${project.id}:${workspace.runId}`}><Mascot bot={taskBot || { name: workspace.botName, color: "#6758d7", mascot: "orbit" }} size="tiny" /><span><strong>{workspace.botName}</strong><small>{workspace.branch}</small></span><b className={`workspace-state workspace-${state}`}>{label}</b><Eye size={13} /></button>; })}</div>}
      {review?.projectId === project.id && <div className="project-review"><div><span><GitBranch size={13} /> {review.workspace ? `${review.workspace.botName} · ${review.branch || "Task branch"}` : review.branch || "Main folder"}</span><b>{review.changes.length ? `${review.changes.length} change${review.changes.length === 1 ? "" : "s"}` : "Ready and clean"}</b></div>{review.workspace && <p className="workspace-review-note">Isolated task workspace · the original project folder has not been changed.</p>}{review.changes.length > 0 && <ul>{review.changes.slice(0, 8).map((change) => <li key={change}>{change}</li>)}</ul>}{review.diff ? <details><summary>See the code diff</summary><pre>{review.diff}{review.truncated ? "\n\n…diff shortened for review" : ""}</pre></details> : <p>No tracked code changes are waiting for review.</p>}</div>}
    ...
    {data?.edits.length ? <section><div className="panel-section-heading"><div><h3>Safety net</h3><p>Restore an agent edit while it is still the newest version of that file</p></div></div><div className="project-edit-list">{data.edits.slice(0, 8).map((edit) => <div key={edit.id}><span><GitBranch size={14} /></span><div><strong>{edit.botName} {edit.operation} {edit.path}</strong><small>{data.projects.find((project) => project.id === edit.projectId)?.name || "Disconnected project"} · {edit.workspaceRunId ? "Isolated task · " : ""}{edit.restoredAt ? "Restored" : relativeTime(edit.createdAt)}</small></div>{edit.reversible && data.projects.some((project) => project.id === edit.projectId) && <button className="project-restore" onClick={() => void restore(edit)} disabled={busy === `restore:${edit.id}`}><RotateCcw size={12} /> Restore</button>}</div>)}</div></section> : null}
    <div className="friendly-note"><ShieldCheck size={17} /><p><strong>Fast work, calm review.</strong><br />Every task gets its own isolated branch, so agents can work at the same time without changing your main folder. Publishing to GitHub always waits for your approval.</p></div>
    {error && <p className="panel-error">{error}</p>}
  </div>;
}

function GitHubConnectorPanel({ status, bots, onRefresh, onNotice }: { status: ConnectorStatus | null; bots: Bot[]; onRefresh: () => Promise<void>; onNotice: (message: string) => void }) {
  const github = status?.github;
  const [busy, setBusy] = useState<string | null>(null), [error, setError] = useState(""), [preview, setPreview] = useState<"notifications" | "issues">("notifications"), [notifications, setNotifications] = useState<GitHubNotificationSummary[]>([]), [issues, setIssues] = useState<GitHubIssueSummary[]>([]), [query, setQuery] = useState("");
  ...
  const connect = () => run("connect-github", async () => { await api("/api/connectors/github/connect", { method: "POST" }); await onRefresh(); onNotice(github?.connected ? "GitHub is ready" : "GitHub sign-in opened — finish it in the browser"); });
  const setAccess = (botId: string, canRead: boolean, canSend: boolean) => run(`github-access-${botId}`, async () => { await api(`/api/connectors/github/access/${botId}`, { method: "PATCH", body: JSON.stringify({ canRead, canSend }) }); await onRefresh(); });
  const checkNotifications = () => run("github-notifications", async () => { const list = await api<GitHubNotificationSummary[]>("/api/connectors/github/notifications?limit=10"); setNotifications(list); setPreview("notifications"); onNotice(list.length ? "GitHub updates are ready" : "Connected — no recent notifications"); });
  const searchIssues = () => run("github-issues", async () => { const list = await api<GitHubIssueSummary[]>(`/api/connectors/github/issues?limit=10&q=${encodeURIComponent(query)}`); setIssues(list); setPreview("issues"); onNotice(list.length ? "GitHub issues are ready" : "No matching issues found"); });
  if (!status) return <section className="github-connector"><div className="github-connect-head"><span className="github-mark"><ConnectorIcon id="github" /></span><div><small>GITHUB PRODUCTIVITY</small><h3>Checking GitHub on this Mac…</h3><p>Looking for an official GitHub sign-in you already trust.</p></div><LoaderCircle className="spinner" size={17} /></div></section>;
  return <section className={`github-connector ${github?.connected ? "connected" : ""}`}>
    <div className="github-connect-head"><span className="github-mark"><ConnectorIcon id="github" /></span><div><small>GITHUB PRODUCTIVITY</small><h3>{github?.connected ? `Ready as @${github.accountLogin}` : github?.installed ? "Bring GitHub into the studio" : "GitHub CLI is needed"}</h3><p>{github?.connected ? "Follow work across repositories, with a separate permission for creating issues." : "Use the official GitHub sign-in already trusted on this Mac—no token copying."}</p></div>{github?.connected ? <span className="connector-ready"><i /> Connected</span> : github?.installed ? <button className="button-primary" onClick={() => void connect()} disabled={busy === "connect-github"}>{busy === "connect-github" || github?.connecting ? <LoaderCircle className="spinner" size={14} /> : <ExternalLink size={14} />} Connect GitHub</button> : <a className="button-secondary" href="https://cli.github.com/" target="_blank" rel="noreferrer">Get GitHub CLI <ExternalLink size={12} /></a>}</div>
    {github?.connected && <><div className="github-checks"><button onClick={() => void checkNotifications()} disabled={busy === "github-notifications"}>{busy === "github-notifications" ? <LoaderCircle className="spinner" size={13} /> : <Bell size={13} />} Check updates</button><form onSubmit={(event) => { event.preventDefault(); void searchIssues(); }}><Search size={13} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search issues…" /><button disabled={busy === "github-issues"}>{busy === "github-issues" ? <LoaderCircle className="spinner" size={13} /> : "Search"}</button></form></div>
      {preview === "notifications" && notifications.length > 0 && <div className="github-preview">{notifications.map((item) => <a key={item.id} href={item.url || `https://github.com/${item.repository}`} target="_blank" rel="noreferrer"><span className={item.unread ? "github-unread" : ""}><Bell size={12} /></span><div><strong>{item.title}</strong><small>{item.repository} · {item.reason.replace(/_/g, " ")}</small></div><time>{relativeTime(item.updatedAt)}</time><ExternalLink size={11} /></a>)}</div>}
      {preview === "issues" && issues.length > 0 && <div className="github-preview">{issues.map((issue) => <a key={issue.id} href={issue.url} target="_blank" rel="noreferrer"><span className={`issue-${issue.state}`}>#{issue.number}</span><div><strong>{issue.title}</strong><small>{issue.repository}{issue.author ? ` · ${issue.author}` : ""}</small></div><time>{relativeTime(issue.updatedAt)}</time><ExternalLink size={11} /></a>)}</div>}
      <div className="github-access"><div className="panel-section-heading"><div><h3>Who can use GitHub</h3><p>Reading activity and preparing new issues stay separate</p></div></div>{bots.map((bot) => { const access = status?.access.find((item) => item.connectorId === "github-cli" && item.botId === bot.id); return <div key={bot.id}><Mascot bot={bot} size="small" /><span><strong>{bot.name}</strong><small>{bot.role}</small></span><button className={access?.canRead ? "on" : ""} onClick={() => void setAccess(bot.id, !access?.canRead, Boolean(access?.canSend))} disabled={busy === `github-access-${bot.id}`}><Eye size={12} /> Read</button><button className={access?.canSend ? "on create" : "create"} onClick={() => void setAccess(bot.id, Boolean(access?.canRead), !access?.canSend)} disabled={busy === `github-access-${bot.id}`}><Plus size={12} /> Create issues</button></div>; })}</div></>}
    {error && <p className="panel-error">{error}</p>}
  </section>;
}

type OAuthProduct = "slack" | "notion";
function OAuthConnectorPanel({ kind, status, bots, onRefresh, onNotice }: { kind: OAuthProduct; status: ConnectorStatus | null; bots: Bot[]; onRefresh: () => Promise<void>; onNotice: (message: string) => void }) {
  const details = kind === "slack"
    ? { name: "Slack", kicker: "TEAM CONVERSATIONS", connectTitle: "Bring Slack into the studio", connectedCopy: "Search the conversations you can already see, then review every message before it is posted.", setupCopy: "Create a Slack app once, then your team gets the same official Connect Slack flow.", docs: "https://api.slack.com/apps", readLabel: "Read", writeLabel: "Post", searchPlaceholder: "Search recent messages…", defaultQuery: "after:yesterday", boundary: "Search follows the connected member’s existing Slack access. Posts are sent by the app only after approval." }
    : { name: "Notion", kicker: "SHARED KNOWLEDGE", connectTitle: "Bring Notion into the studio", connectedCopy: "Find current pages, read useful context, and review every note before it is added.", setupCopy: "Create a public Notion integration once, then choose exactly which pages OpenBot may use.", docs: "https://www.notion.so/my-integrations", readLabel: "Read", writeLabel: "Add notes", searchPlaceholder: "Search shared pages…", defaultQuery: "", boundary: "OpenBot sees only the pages selected or shared during Notion sign-in. Updates append content; they never replace a page." };
  const connector = status?.[kind], connection = status?.connections.find((item) => item.id === kind);
  const [clientId, setClientId] = useState(""), [clientSecret, setClientSecret] = useState(""), [query, setQuery] = useState(details.defaultQuery);
  const [busy, setBusy] = useState<string | null>(null), [error, setError] = useState(""), [slackResults, setSlackResults] = useState<SlackMessageSummary[]>([]), [notionResults, setNotionResults] = useState<NotionPageSummary[]>([]);
  const run = async (key: string, operation: () => Promise<void>) => { setBusy(key); setError(""); try { await operation(); } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); } finally { setBusy(null); } };
  const configure = (event: FormEvent) => { event.preventDefault(); void run("configure", async () => { await api(`/api/connectors/${kind}/config`, { method: "POST", body: JSON.stringify({ clientId, clientSecret }) }); setClientSecret(""); await onRefresh(); onNotice(`${details.name} connection saved privately`); }); };
  const connect = () => run("connect", async () => { const next = await api<{ url: string }>(`/api/connectors/${kind}/connect`, { method: "POST" }); onNotice(`Opening ${details.name} sign-in…`); window.location.assign(next.url); });
  const check = () => run("check", async () => { await api(`/api/connectors/${kind}/health`, { method: "POST" }); await onRefresh(); onNotice(`${details.name} is connected and ready`); });
  const preview = () => run("preview", async () => {
    const results = await api<SlackMessageSummary[] | NotionPageSummary[]>(`/api/connectors/${kind}/preview?q=${encodeURIComponent(query)}`);
    if (kind === "slack") setSlackResults(results as SlackMessageSummary[]); else setNotionResults(results as NotionPageSummary[]);
    await onRefresh(); onNotice(results.length ? `${details.name} results are ready` : `Connected — no matching ${kind === "slack" ? "messages" : "pages"}`);
  });
  const setAccess = (botId: string, canRead: boolean, canSend: boolean) => run(`access-${botId}`, async () => { await api(`/api/connectors/${kind}/access/${botId}`, { method: "PATCH", body: JSON.stringify({ canRead, canSend }) }); await onRefresh(); });
  const disconnect = () => run("disconnect", async () => { await api(`/api/connectors/${kind}/disconnect`, { method: "POST" }); setSlackResults([]); setNotionResults([]); await onRefresh(); onNotice(`${details.name} disconnected`); });
  const copyCallback = async () => { if (!connector?.callbackUrl) return; await navigator.clipboard.writeText(connector.callbackUrl); onNotice("Callback address copied"); };
  if (!status || !connector) return null;
  return <section className={`oauth-connector oauth-${kind} ${connector.connected ? "connected" : ""}`}>
    <div className="oauth-connector-head"><span className="oauth-product-mark"><ConnectorIcon id={kind} /></span><div><small>{details.kicker}</small><h3>{connector.connected ? `${details.name} is ready${connector.accountName ? ` · ${connector.accountName}` : ""}` : details.connectTitle}</h3><p>{connector.connected ? details.connectedCopy : connector.configured ? `Your private ${details.name} connection is ready for sign-in.` : details.setupCopy}</p></div>{connector.connected ? <span className="connector-ready"><i /> Connected</span> : connector.configured ? <button className="button-primary" onClick={() => void connect()} disabled={busy === "connect"}>{busy === "connect" || connector.oauthInProgress ? <LoaderCircle className="spinner" size={14} /> : <ExternalLink size={14} />} {connector.oauthInProgress ? "Waiting…" : `Connect ${details.name}`}</button> : null}</div>
    {!connector.configured && !connector.managedClient && <form className="oauth-setup-form" onSubmit={configure}><div className="oauth-setup-intro"><span><ShieldCheck size={15} /></span><div><strong>Self-hosted setup</strong><small>The public release can bundle this once so everyone else gets one-click sign-in.</small></div><a href={details.docs} target="_blank" rel="noreferrer">Create app <ExternalLink size={11} /></a></div><div className="oauth-fields"><label className="field"><span>Client ID</span><input value={clientId} onChange={(event) => setClientId(event.target.value)} required /></label><label className="field"><span>Client secret</span><input type="password" value={clientSecret} onChange={(event) => setClientSecret(event.target.value)} autoComplete="new-password" required /></label></div><div className="callback-row"><span><strong>Return address</strong><code>{connector.callbackUrl}</code></span><button type="button" onClick={() => void copyCallback()}><Copy size={14} /> Copy</button></div><button className="button-primary" disabled={busy === "configure"}>{busy === "configure" ? <LoaderCircle className="spinner" size={14} /> : <ShieldCheck size={14} />} Save privately</button></form>}
    {connector.connected && <><div className="oauth-connector-tools"><button onClick={() => void check()} disabled={busy === "check"}>{busy === "check" ? <LoaderCircle className="spinner" size={13} /> : <RefreshCw size={13} />} Check connection</button><form onSubmit={(event) => { event.preventDefault(); void preview(); }}><Search size={13} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={details.searchPlaceholder} /><button disabled={busy === "preview"}>{busy === "preview" ? <LoaderCircle className="spinner" size={13} /> : "Search"}</button></form></div>
      {kind === "slack" && slackResults.length > 0 && <div className="oauth-preview-list">{slackResults.map((item) => item.permalink ? <a key={`${item.channelId}:${item.timestamp}`} href={item.permalink} target="_blank" rel="noreferrer"><span className="mini-service-icon"><ConnectorIcon id="slack" /></span><span><strong>#{item.channelName} · {item.author}</strong><small>{item.text}</small></span><ExternalLink size={11} /></a> : <div key={`${item.channelId}:${item.timestamp}`}><span className="mini-service-icon"><ConnectorIcon id="slack" /></span><span><strong>#{item.channelName} · {item.author}</strong><small>{item.text}</small></span></div>)}</div>}
      {kind === "notion" && notionResults.length > 0 && <div className="oauth-preview-list">{notionResults.map((item) => <a key={item.id} href={item.url} target="_blank" rel="noreferrer"><span className="mini-service-icon"><ConnectorIcon id="notion" /></span><span><strong>{item.title}</strong><small>Edited {relativeTime(item.lastEditedAt)}</small></span><ExternalLink size={11} /></a>)}</div>}
      <div className="oauth-boundary"><ShieldCheck size={14} /><span>{details.boundary}</span></div>
      <div className="github-access oauth-access"><div className="panel-section-heading"><div><h3>Who can use {details.name}</h3><p>Reading and preparing changes stay separate</p></div></div>{bots.map((bot) => { const access = status.access.find((item) => item.connectorId === kind && item.botId === bot.id); return <div key={bot.id}><Mascot bot={bot} size="small" /><span><strong>{bot.name}</strong><small>{bot.role}</small></span><button className={access?.canRead ? "on" : ""} onClick={() => { const canRead = !access?.canRead; void setAccess(bot.id, canRead, canRead && Boolean(access?.canSend)); }} disabled={busy === `access-${bot.id}`}><Eye size={12} /> {details.readLabel}</button><button className={access?.canSend ? "on create" : "create"} onClick={() => void setAccess(bot.id, true, !access?.canSend)} disabled={busy === `access-${bot.id}`}><Plus size={12} /> {details.writeLabel}</button></div>; })}</div>
      <button className="disconnect-button" onClick={() => { if (window.confirm(`Disconnect ${details.name} from OpenBot? The app details stay saved so you can reconnect later.`)) void disconnect(); }} disabled={busy === "disconnect"}>Disconnect {details.name}</button></>}
    {connection?.lastError && !connector.connected && <p className="oauth-attention"><CircleAlert size={14} /> {connection.lastError}</p>}
    {error && <p className="panel-error">{error}</p>}
  </section>;
}
...
function ConnectorPanel({ status, bots, onRefresh, onNotice, onStartWorkflow }: { status: ConnectorStatus | null; bots: Bot[]; onRefresh: () => Promise<void>; onNotice: (message: string) => void; onStartWorkflow: (prompt: string) => Promise<void> }) {
  ...
  const slackReady = Boolean(status?.slack.connected), notionReady = Boolean(status?.notion.connected), githubReady = Boolean(status?.github.connected);
  ...
  const friendlyAction = (action: string) => action === "gmail_search" ? "Searched the inbox" : action === "gmail_read" ? "Read an email" : action === "gmail_send" ? "Prepared or sent an email" : action === "google_drive_search" ? "Searched Google Drive" : action === "google_drive_read" ? "Read a Drive file" : action === "google_calendar_agenda" ? "Checked the calendar" : action === "github_notifications" ? "Checked GitHub updates" : action === "github_issues" ? "Searched GitHub issues" : action === "github_issue_create" ? "Prepared or created a GitHub issue" : action === "slack_search" ? "Searched Slack" : action === "slack_read" ? "Read a Slack conversation" : action === "slack_post" ? "Prepared or posted in Slack" : action === "notion_search" ? "Searched Notion" : action === "notion_read" ? "Read a Notion page" : action === "notion_update" ? "Prepared or added to Notion" : action === "connected" ? "Connected an app" : action === "disconnected" ? "Disconnected an app" : action;
  const friendlyEventSummary = (event: ConnectorStatus["events"][number]) => {
    ...
    if (event.status === "failed" && event.action.startsWith("slack")) return status?.slack.lastError || "Slack needs a quick connection check before this can run.";
    if (event.status === "failed" && event.action.startsWith("notion")) return status?.notion.lastError || "Notion needs a quick connection check before this can run.";
  ...
  return <div className="connector-panel">
    <div className="connector-hero">
      <div className="connector-hero-copy"><span className="control-kicker"><Sparkles size={13} /> A calmer way to get things done</span><h3>Connect once. Give each teammate only what they need.</h3><p>Mail, conversations, knowledge and code come into one studio—while every external write still waits for your okay.</p></div>
      <div className="connector-stage" aria-hidden="true"><Mascot bot={{ name: "Pip", color: "#6657d8", mascot: "orbit", status: status?.catalog.some((item) => item.connected) ? "ready" : "working" }} size="large" /><span className="connector-float connector-float-gmail"><ConnectorIcon id="slack" /></span><span className="connector-float connector-float-drive"><ConnectorIcon id="notion" /></span><span className="connector-line" /></div>
    </div>

    <section><div className="panel-section-heading"><div><h3>Apps your team can use</h3><p>Working connections are clearly separated from the roadmap</p></div>{connected && <span className="connector-ready"><i /> Google ready</span>}</div><div className="connector-catalog">{status?.catalog.filter((entry) => entry.id !== "github").map((entry) => <article key={entry.id} className={`connector-card ${entry.connected ? "connected" : ""} ${entry.availability === "next" ? "coming" : ""}`}><span className={`connector-logo connector-logo-${entry.id}`}><ConnectorIcon id={entry.id} /></span><div className="connector-card-main"><div className="connector-card-title"><h4>{entry.name}</h4><b>{entry.connected ? "Connected" : entry.badge}</b></div><p>{entry.description}</p><div className="connector-capabilities">{entry.capabilities.map((capability) => <span key={capability}>{capability}</span>)}</div></div></article>) || <div className="connector-loading"><LoaderCircle className="spinner" /> Checking your apps…</div>}</div></section>
    ...
    <GitHubConnectorPanel status={status} bots={bots} onRefresh={onRefresh} onNotice={onNotice} />
    <OAuthConnectorPanel kind="slack" status={status} bots={bots} onRefresh={onRefresh} onNotice={onNotice} />
    <OAuthConnectorPanel kind="notion" status={status} bots={bots} onRefresh={onRefresh} onNotice={onNotice} />
    {(slackReady || notionReady) && <section><div className="panel-section-heading"><div><h3>Connected workflows</h3><p>Turn live conversations and knowledge into a checked result—not another pile of summaries</p></div></div><div className="workflow-starters"><button disabled={!slackReady} onClick={() => void onStartWorkflow("Use @slack to catch me up on the last working day. Search for decisions, direct questions, blockers, deadlines, and promises involving me or my active projects. Read the relevant conversation context, remove duplicates, separate facts from inference, include normal Slack links, and deliver one short priority brief. Do not post anything.")}><span><ConnectorIcon id="slack" /></span><div><strong>Slack catch-up</strong><small>Decisions, asks and blockers—with source links</small></div><ArrowUp size={14} /></button><button disabled={!notionReady} onClick={() => void onStartWorkflow("Use @notion to find the most recently edited pages relevant to current projects. Read the useful pages, identify decisions, owners, dates, unanswered questions, and stale information, then deliver one concise knowledge brief with normal Notion links. Do not update any page.")}><span><ConnectorIcon id="notion" /></span><div><strong>Knowledge brief</strong><small>Current decisions and open questions from shared pages</small></div><ArrowUp size={14} /></button><button disabled={!slackReady || !notionReady || !githubReady} onClick={() => void onStartWorkflow("Use @slack, @notion, and @github to make a verified project pulse. Compare recent conversation decisions, shared project notes, open issues, and repository notifications. Resolve obvious duplicates, flag disagreements or stale notes, cite normal source links, and give me priorities, owners, risks, and next actions. Read only; do not post, create, or update anything.")}><span><Sparkles size={16} /></span><div><strong>Project pulse</strong><small>Cross-check Slack, Notion and GitHub in one view</small></div><ArrowUp size={14} /></button></div></section>}
    {status?.events.length ? <section><div className="panel-section-heading"><div><h3>Recent app activity</h3><p>A clear trail without exposing private message or page contents</p></div></div><div className="connector-events">{status.events.slice(0, 8).map((event) => <div key={event.id}><span className={`event-state event-${event.status}`}>{event.status === "completed" ? <Check size={11} /> : event.status === "waiting" ? <Clock3 size={11} /> : "!"}</span><span><strong>{event.botName ? `${event.botName} · ` : ""}{friendlyAction(event.action)}</strong><small>{friendlyEventSummary(event)}</small></span><time>{relativeTime(event.createdAt)}</time></div>)}</div></section> : null}
...
function RemotePanel({ bots, installPrompt, onInstalled, onNotice }: { bots: Bot[]; installPrompt: InstallPrompt | null; onInstalled: () => void; onNotice: (message: string) => void }) {
  ...
  const shareWithIPhone = async (value: string) => {
    if (navigator.share) {
      try { await navigator.share({ title: "OpenBot", text: "Connect my iPhone to this OpenBot studio.", url: value }); return; }
...
function BotPanel({ bot, provider, apps = [], onSave, onOpenTeach }: { bot: Bot; provider: ProviderStatus | null; apps?: ConnectorStatus["access"]; onSave: (id: string, patch: Partial<Bot>) => Promise<void>; onOpenTeach: () => void }) {
  const [form, setForm] = useState({ role: bot.role, instructions: bot.instructions, model: bot.model, weeklyTokenBudget: bot.weeklyTokenBudget, computerEnabled: bot.computerEnabled, browserEnabled: bot.browserEnabled, mascot: bot.mascot, color: bot.color });
  ...
  const gmail = apps.find((access) => access.botId === bot.id && access.service === "gmail"), drive = apps.find((access) => access.botId === bot.id && access.service === "google-drive"), calendar = apps.find((access) => access.botId === bot.id && access.service === "google-calendar"), github = apps.find((access) => access.botId === bot.id && access.service === "github"), slack = apps.find((access) => access.botId === bot.id && access.service === "slack"), notion = apps.find((access) => access.botId === bot.id && access.service === "notion");
  ...
  return <form className="bot-editor" onSubmit={submit}>
    <div className="bot-hero" style={{ "--bot-color": form.color } as React.CSSProperties}><Mascot bot={preview} size="large" /><h3>{bot.name}</h3><p>{bot.status === "working" ? "Busy making progress" : bot.status === "waiting" ? "Patiently waiting for you" : "Ready for something new"}</p><div className="capability-pills"><span className={!form.computerEnabled ? "off" : ""}><Cpu size={12} /> {form.computerEnabled ? "private computer" : "computer off"}</span><span className={!form.browserEnabled ? "off" : ""}><Globe2 size={12} /> {form.browserEnabled ? "own browser" : "browser off"}</span><span className={!bot.macAccessEnabled ? "off" : ""}><HardDrive size={12} /> {bot.macAccessEnabled ? "Mac files & apps" : "Studio Mac access off"}</span>{gmail?.canRead && <span><Mail size={12} /> Gmail{gmail.canSend ? " + send" : ""}</span>}{drive?.canRead && <span><FolderOpen size={12} /> Drive</span>}{calendar?.canRead && <span><Clock3 size={12} /> Calendar</span>}{github?.canRead && <span><ConnectorIcon id="github" /> GitHub{github.canSend ? " + issues" : ""}</span>}{slack?.canRead && <span><ConnectorIcon id="slack" /> Slack{slack.canSend ? " + post" : ""}</span>}{notion?.canRead && <span><ConnectorIcon id="notion" /> Notion{notion.canSend ? " + notes" : ""}</span>}</div></div>
    <fieldset className="appearance-editor"><legend>Appearance</legend><p>Pick a shape and color. Your teammate updates everywhere, including iPhone.</p><div className="mascot-picker">{MASCOT_KINDS.map((kind) => <button type="button" aria-label={`Use ${kind} shape`} key={kind} className={form.mascot === kind ? "chosen" : ""} onClick={() => setForm({ ...form, mascot: kind })}><Mascot bot={{ ...preview, mascot: kind }} size="small" /></button>)}</div><div className="color-picker">{MASCOT_COLORS.map((value) => <button type="button" aria-label={`Use ${value} color`} key={value} className={form.color === value ? "chosen" : ""} style={{ background: value }} onClick={() => setForm({ ...form, color: value })} />)}<label className="custom-color" title="Choose any color"><input aria-label="Choose a custom mascot color" type="color" value={form.color} onChange={(event) => setForm({ ...form, color: event.target.value })} /><span>+</span></label></div></fieldset>
...
type RoutineInput = { name: string; botId: string; threadId: string; prompt: string; intervalMinutes: number; enabled: boolean; triggerType: AutomationTriggerType; triggerConfig: Routine["triggerConfig"] };
type RoutineSaveResult = Routine & { webhook?: { url: string; secret: string } };

function routineTriggerLabel(routine: Routine) {
  if (routine.triggerType === "schedule") return routineScheduleLabel(routine.intervalMinutes);
  if (routine.triggerType === "github") return `GitHub · ${routine.triggerConfig.githubEvent || "event"}${routine.triggerConfig.githubAction ? ` / ${routine.triggerConfig.githubAction}` : ""}`;
  if (routine.triggerType === "calendar") return `${routine.triggerConfig.minutesBefore ?? 15} min before a Calendar event`;
  return routine.triggerConfig.eventName ? `Webhook · ${routine.triggerConfig.eventName}` : "Signed webhook";
}
...
function RoutinesPanel({ routines, events, alerts, bots, onCreate, onUpdate, onToggle, onDelete, onRun, onReplay, onRotateSecret, onResolveAlert, onOpenResult }: {
  routines: Routine[]; events: AutomationEvent[]; alerts: AutomationAlert[]; bots: Bot[];
  onCreate: (input: RoutineInput) => Promise<RoutineSaveResult>; onUpdate: (routine: Routine, input: RoutineInput) => Promise<RoutineSaveResult>;
  onToggle: (routine: Routine) => Promise<void>; onDelete: (routine: Routine) => Promise<void>; onRun: (routine: Routine) => Promise<void>;
  onReplay: (event: AutomationEvent) => Promise<void>; onRotateSecret: (routine: Routine) => Promise<RoutineSaveResult>; onResolveAlert: (alert: AutomationAlert) => Promise<void>; onOpenResult: (routine: Routine) => void;
}) {
  ...
  const [schedule, setSchedule] = useState<"5" | "60" | "1440" | "10080" | "custom">("1440"), [customAmount, setCustomAmount] = useState(2), [customUnit, setCustomUnit] = useState<"minutes" | "hours" | "days">("hours");
  const [triggerType, setTriggerType] = useState<AutomationTriggerType>("schedule"), [eventName, setEventName] = useState(""), [githubEvent, setGithubEvent] = useState("issues"), [githubAction, setGithubAction] = useState("opened"), [repository, setRepository] = useState(""), [titleContains, setTitleContains] = useState(""), [minutesBefore, setMinutesBefore] = useState(15);
  const [enabled, setEnabled] = useState(true), [saving, setSaving] = useState(false), [editing, setEditing] = useState<Routine | null>(null), [openHistory, setOpenHistory] = useState<string | null>(null);
  const [createdHook, setCreatedHook] = useState<{ name: string; url: string; secret: string; type: AutomationTriggerType } | null>(null);
  ...
  const intervalValid = triggerType !== "schedule" || (intervalMinutes >= 5 && intervalMinutes <= 43_200);
  const copy = (value: string) => void navigator.clipboard.writeText(value);
  const reset = () => { setName(""); setPrompt(""); setSchedule("1440"); setCustomAmount(2); setCustomUnit("hours"); setTriggerType("schedule"); setEventName(""); setGithubEvent("issues"); setGithubAction("opened"); setRepository(""); setTitleContains(""); setMinutesBefore(15); setEnabled(true); setEditing(null); setCreating(false); };
  const startCreate = () => { reset(); setCreatedHook(null); setCreating(true); };
  const startEdit = (routine: Routine) => {
    setEditing(routine); setCreating(true); setCreatedHook(null); setName(routine.name); setPrompt(routine.prompt); setBotId(routine.botId); setEnabled(routine.enabled); setTriggerType(routine.triggerType);
    setEventName(routine.triggerConfig.eventName || ""); setGithubEvent(routine.triggerConfig.githubEvent || "issues"); setGithubAction(routine.triggerConfig.githubAction || ""); setRepository(routine.triggerConfig.repository || ""); setTitleContains(routine.triggerConfig.titleContains || ""); setMinutesBefore(routine.triggerConfig.minutesBefore ?? 15);
    const preset = [5, 60, 1440, 10080].includes(routine.intervalMinutes) ? String(routine.intervalMinutes) as typeof schedule : "custom";
    setSchedule(preset);
    if (preset === "custom") {
      const unit = routine.intervalMinutes % 1440 === 0 ? "days" : routine.intervalMinutes % 60 === 0 ? "hours" : "minutes";
      setCustomUnit(unit); setCustomAmount(routine.intervalMinutes / (unit === "days" ? 1440 : unit === "hours" ? 60 : 1));
    }
  };
  const submit = async (event: FormEvent) => {
    ...
    setSaving(true);
    const triggerConfig = triggerType === "github" ? { githubEvent, ...(githubAction ? { githubAction } : {}), ...(repository ? { repository } : {}) } : triggerType === "webhook" ? { ...(eventName ? { eventName } : {}) } : triggerType === "calendar" ? { ...(titleContains ? { titleContains } : {}), minutesBefore } : {};
    try {
      const input = { name, prompt, botId: selectedBot.id, threadId: selectedBot.threadId, intervalMinutes: triggerType === "schedule" ? intervalMinutes : 1440, enabled, triggerType, triggerConfig };
      const result = editing ? await onUpdate(editing, input) : await onCreate(input);
      if (result.webhook) setCreatedHook({ name: result.name, ...result.webhook, type: result.triggerType });
      reset();
    } finally { setSaving(false); }
  };
  const lastStatus = (routine: Routine) => routine.pausedReason || (routine.lastStatus === "completed" ? "Last run finished" : routine.lastStatus === "failed" ? "Needs attention" : "Ready for its first event");
  return <div className="routines-view">
    <section className="routine-hero"><div className="routine-orbit"><Workflow size={22} /><i /><i /><i /></div><span><b>Dependable automations</b><h3>One event. One checked result.</h3><p>Schedules, Calendar and signed hooks—with retries you can understand.</p></span></section>

    {alerts.length > 0 && <section className="automation-inbox"><header><span><Bell size={15} /></span><div><strong>Needs your attention</strong><small>{alerts.length} automation {alerts.length === 1 ? "item" : "items"} to review</small></div></header>{alerts.map((alert) => { const routine = routines.find((item) => item.id === alert.routineId); const sourceEvent = events.find((item) => item.id === alert.eventId); return <article key={alert.id}><span className={`automation-alert-icon alert-${alert.kind}`}>{alert.kind === "approval" ? <ShieldCheck size={14} /> : <CircleAlert size={14} />}</span><div><strong>{alert.routineName}</strong><p>{alert.message}</p>{alert.repairHint && <small>{alert.repairHint}</small>}</div><div>{routine && sourceEvent && ["failed", "cancelled", "rate_limited"].includes(sourceEvent.status) && <button onClick={() => void onReplay(sourceEvent)}><RefreshCw size={12} /> Retry</button>}{routine && <button onClick={() => onOpenResult(routine)}><ExternalLink size={12} /> Open</button>}<button onClick={() => void onResolveAlert(alert)}><Check size={12} /> Clear</button></div></article>; })}</section>}

    {createdHook && <section className="hook-ready"><header><span><Check size={16} /></span><div><strong>{createdHook.name} is ready</strong><small>Copy this secret now. OpenBot will not show it again.</small></div><button onClick={() => setCreatedHook(null)} aria-label="Close"><X size={14} /></button></header><label><span>Hook URL</span><code>{createdHook.url}</code><button onClick={() => copy(createdHook.url)}><Copy size={13} /> Copy</button></label><label><span>{createdHook.type === "github" ? "GitHub webhook secret" : "Signing secret"}</span><code>{createdHook.secret}</code><button onClick={() => copy(createdHook.secret)}><Copy size={13} /> Copy</button></label><p>{createdHook.type === "github" ? "Use application/json and enable SSL verification. GitHub signs deliveries with this secret." : "Send x-openbot-signature as sha256=HMAC_SHA256(raw request body, secret), plus a stable x-openbot-event-id."}</p></section>}

    {routines.length > 0 && <div className="routine-list">{routines.map((routine) => {
      const bot = bots.find((item) => item.id === routine.botId) || bots[0]!;
      const routineEvents = events.filter((item) => item.routineId === routine.id);
      return <article key={routine.id} className={`routine-card ${routine.enabled ? "is-on" : ""} ${routine.pausedReason ? "needs-attention" : ""}`}>
        <div className="routine-row"><Mascot bot={bot} size="small" /><div><strong>{routine.name}</strong><span>{routine.botName} · {routineTriggerLabel(routine)}</span><small>{routine.enabled ? lastStatus(routine) : routine.pausedReason || "Paused"}{routine.nextRunAt && routine.enabled ? ` · next ${relativeTime(routine.nextRunAt) === "Now" ? "soon" : `in ${routineStartsInLabel(Math.max(5, Math.round((new Date(routine.nextRunAt).getTime() - Date.now()) / 60_000))).replace(/^Starts /, "")}`}` : ""}</small></div><button className="routine-test" onClick={() => { if (window.confirm(`Test “${routine.name}” now? This uses the real permissions and can perform real actions.`)) void onRun(routine); }} title="Test with real permissions"><Play size={12} /> Test</button><button className={`toggle ${routine.enabled ? "on" : ""}`} onClick={() => void onToggle(routine)} aria-label={`${routine.enabled ? "Pause" : "Start"} ${routine.name}`}><span /></button></div>
        <div className="routine-health"><span className={routine.consecutiveFailures ? "warning" : "healthy"}>{routine.consecutiveFailures ? `${routine.consecutiveFailures} failed in a row` : "Healthy"}</span><span>{routine.runCount} runs</span>{routine.deduplicatedCount > 0 && <span>{routine.deduplicatedCount} duplicate{routine.deduplicatedCount === 1 ? "" : "s"} stopped</span>}{["github", "webhook"].includes(routine.triggerType) && <><button onClick={() => copy(`${window.location.origin}/api/automation-hooks/${routine.id}`)}><Copy size={11} /> Hook URL</button><button onClick={() => { if (window.confirm("Create a new signing secret? The previous secret will stop working immediately.")) void onRotateSecret(routine).then((result) => { if (result.webhook) setCreatedHook({ name: result.name, ...result.webhook, type: result.triggerType }); }); }}><RotateCcw size={11} /> New secret</button></>}</div>
        <div className="routine-actions"><button onClick={() => startEdit(routine)}><Settings2 size={13} /> Edit</button><button onClick={() => setOpenHistory(openHistory === routine.id ? null : routine.id)}><Clock3 size={13} /> {openHistory === routine.id ? "Hide activity" : `Activity${routineEvents.length ? ` · ${routineEvents.length}` : ""}`}</button><button className="danger" onClick={() => { if (window.confirm(`Delete “${routine.name}”? Its past conversation results will stay available.`)) void onDelete(routine); }}><Trash2 size={13} /> Delete</button></div>
        {openHistory === routine.id && <div className="routine-history">{routineEvents.length ? routineEvents.map((item) => <div key={item.id} className={`routine-history-row history-${item.status}`}><span className="history-state">{item.status === "completed" ? <Check size={12} /> : ["failed", "cancelled", "rate_limited"].includes(item.status) ? "!" : item.status === "waiting" ? <ShieldCheck size={12} /> : <LoaderCircle className={item.status === "running" ? "spinner" : ""} size={12} />}</span><span><strong>{automationStatusLabel(item.status)}{item.attempt > 1 ? ` · try ${item.attempt}` : ""}</strong><small>{item.payloadSummary}</small>{item.error && <small className="history-error">{item.error}</small>}</span><time>{relativeTime(item.finishedAt || item.receivedAt)}</time>{["failed", "cancelled", "rate_limited"].includes(item.status) && <button onClick={() => void onReplay(item)}><RefreshCw size={12} /> Retry</button>}<button onClick={() => onOpenResult(routine)}><ExternalLink size={12} /> Open</button></div>) : <div className="routine-history-empty"><Clock3 size={16} /> No events yet. Test it when you are ready.</div>}</div>}
      </article>;
    })}</div>}

    {creating ? <form className="routine-form" onSubmit={submit}>
      <header className="routine-form-head"><span>{editing ? <Settings2 size={17} /> : <Sparkles size={17} />}</span><div><strong>{editing ? "Edit automation" : "New automation"}</strong><small>{editing ? "Change the job, trigger or owner without losing its history." : "Choose what wakes your teammate and what should happen next."}</small></div></header>
      <label className="field"><span>Name</span><input autoFocus value={name} onChange={(e) => setName(e.target.value)} placeholder="New issue triage" required /></label>
      <fieldset className="routine-fieldset"><legend>What starts it?</legend><div className="trigger-presets">{[
        { value: "schedule" as const, label: "Schedule", icon: <Clock3 size={14} /> }, { value: "calendar" as const, label: "Calendar", icon: <CalendarDays size={14} /> },
        { value: "github" as const, label: "GitHub", icon: <GitBranch size={14} /> }, { value: "webhook" as const, label: "Webhook", icon: <Webhook size={14} /> },
      ].map((item) => <button key={item.value} type="button" className={triggerType === item.value ? "selected" : ""} onClick={() => setTriggerType(item.value)}>{item.icon}<span>{item.label}</span></button>)}</div></fieldset>
      {triggerType === "schedule" && <fieldset className="routine-fieldset"><legend>How often?</legend><div className="schedule-presets">{[{ value: "5", label: "5 min" }, { value: "60", label: "Hourly" }, { value: "1440", label: "Daily" }, { value: "10080", label: "Weekly" }, { value: "custom", label: "Custom" }].map((item) => <button key={item.value} type="button" className={schedule === item.value ? "selected" : ""} onClick={() => setSchedule(item.value as typeof schedule)}>{item.label}</button>)}</div>{schedule === "custom" && <div className="custom-schedule"><input type="number" min={customUnit === "minutes" ? 5 : 1} max={customUnit === "days" ? 30 : customUnit === "hours" ? 720 : 43200} value={customAmount} onChange={(e) => setCustomAmount(Number(e.target.value))} aria-label="Repeat interval" /><select value={customUnit} onChange={(e) => setCustomUnit(e.target.value as typeof customUnit)} aria-label="Repeat unit"><option value="minutes">minutes</option><option value="hours">hours</option><option value="days">days</option></select></div>}{!intervalValid && <small className="routine-error">Choose a repeat time from 5 minutes to 30 days.</small>}</fieldset>}
      {triggerType === "github" && <fieldset className="routine-fieldset"><legend>Which GitHub activity?</legend><div className="trigger-fields"><label><span>Event</span><select value={githubEvent} onChange={(e) => setGithubEvent(e.target.value)}><option value="issues">Issues</option><option value="pull_request">Pull requests</option><option value="push">Pushes</option><option value="release">Releases</option></select></label><label><span>Action</span><input value={githubAction} onChange={(e) => setGithubAction(e.target.value)} placeholder="opened (optional)" /></label><label className="wide"><span>Repository</span><input value={repository} onChange={(e) => setRepository(e.target.value)} placeholder="owner/repository (optional)" /></label></div></fieldset>}
      {triggerType === "webhook" && <fieldset className="routine-fieldset"><legend>Which signed event?</legend><div className="trigger-fields one"><label><span>Event name</span><input value={eventName} onChange={(e) => setEventName(e.target.value)} placeholder="invoice.paid (optional)" /></label></div><small className="routine-help">You’ll get a private signing secret once. Repeated event IDs never start the job twice.</small></fieldset>}
      {triggerType === "calendar" && <fieldset className="routine-fieldset"><legend>Which Calendar events?</legend><div className="trigger-fields"><label><span>Minutes before</span><input type="number" min="0" max="1440" value={minutesBefore} onChange={(e) => setMinutesBefore(Number(e.target.value))} /></label><label><span>Title contains</span><input value={titleContains} onChange={(e) => setTitleContains(e.target.value)} placeholder="Launch review (optional)" /></label></div><small className="routine-help">Uses this teammate’s existing Google Calendar read permission.</small></fieldset>}
      ...
      <label className="field"><span>What should happen?</span><textarea rows={5} value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Read the event, check the related context and post a concise next action…" required /></label>
      <label className="routine-start"><input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} /><span><b>Turn it on</b><small>{enabled ? triggerType === "schedule" ? routineStartsInLabel(intervalMinutes) : "It will wait safely for a matching event" : "Keep it as a paused draft"}</small></span><i><Check size={12} /></i></label>
      <div className="routine-preview">{triggerType === "schedule" ? <Clock3 size={15} /> : triggerType === "calendar" ? <CalendarDays size={15} /> : <Webhook size={15} />}<p><strong>{selectedBot?.name || "Your teammate"}</strong> will run “{name.trim() || "your automation"}” when {triggerType === "schedule" ? routineScheduleLabel(intervalMinutes).toLowerCase() : triggerType === "calendar" ? `a matching event is ${minutesBefore} minutes away` : triggerType === "github" ? "a signed GitHub event matches these filters" : "a correctly signed webhook arrives"}. Test runs use the same real permissions.</p></div>
      <div className="form-actions"><button type="button" className="button-secondary" onClick={reset}>Cancel</button><button className="button-primary" disabled={!intervalValid || saving}>{saving ? <><LoaderCircle className="spinner" size={15} /> Saving…</> : editing ? "Save changes" : "Create automation"}</button></div>
    </form> : <button className="add-routine" onClick={startCreate}><Plus size={17} /> Add an automation</button>}
  </div>;
}
...
function TeachPanel({ bot, onUse, onNotice }: { bot: Bot; onUse: (workflow: TaughtWorkflow) => Promise<void>; onNotice: (message: string) => void }) {
  ...
  const [editingWorkflow, setEditingWorkflow] = useState<TaughtWorkflow | null>(null), [editName, setEditName] = useState(""), [editUrl, setEditUrl] = useState("");
  ...
  const edit = (workflow: TaughtWorkflow) => { setEditingWorkflow(workflow); setEditName(workflow.name); setEditUrl(workflow.startUrl); setError(""); };
  const saveEdit = async (event: FormEvent) => { event.preventDefault(); if (!editingWorkflow) return; setBusy(true); setError(""); try { await api(`/api/workflows/${editingWorkflow.id}`, { method: "PATCH", body: JSON.stringify({ name: editName, startUrl: editUrl }) }); setEditingWorkflow(null); await refresh(); onNotice("Skill updated"); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setBusy(false); } };
  const remove = async (workflow: TaughtWorkflow) => { if (!window.confirm(`Delete /${workflow.skillSlug}? This removes the learned steps from ${bot.name}.`)) return; setBusy(true); setError(""); try { await api(`/api/workflows/${workflow.id}`, { method: "DELETE" }); if (editingWorkflow?.id === workflow.id) setEditingWorkflow(null); await refresh(); onNotice("Skill deleted"); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setBusy(false); } };
  return <div className="teach-panel"><div className={`teach-hero ${recording ? "recording" : ""}`}><div className="teach-visual"><Mascot bot={{ ...bot, status: recording ? "working" : "ready" }} size="large" /><span className="teach-pointer">↖</span></div><h3>{recording ? `${bot.name} is watching the steps` : `Show ${bot.name} how you do it`}</h3><p>{recording ? `${stepCount} meaningful action${stepCount === 1 ? "" : "s"} captured. Secrets are replaced with placeholders.` : "Open a visible browser, complete the task once, then save it as a reusable skill."}</p></div>
    ...
    <section><div className="panel-section-heading"><div><h3>Learned skills</h3><p>Type / in chat, or manage each reusable workflow here</p></div></div>{workflows.length ? <div className="workflow-list">{workflows.map((workflow) => <div key={workflow.id}><WandSparkles size={18} /><span><strong>/{workflow.skillSlug}</strong><small>{workflow.name} · {workflow.stepCount} steps · {new URL(workflow.startUrl).hostname}</small></span><div className="workflow-actions"><button onClick={() => void onUse(workflow)}><Play size={12} /> Use</button><button onClick={() => edit(workflow)}><Settings2 size={12} /> Edit</button><button className="danger" onClick={() => void remove(workflow)} aria-label={`Delete ${workflow.name}`}><Trash2 size={12} /></button></div></div>)}</div> : <div className="empty-panel compact"><PanelTop size={28} /><h3>Nothing taught yet</h3><p>Your first demonstrated workflow will appear here.</p></div>}</section>
    {editingWorkflow && <form className="workflow-edit-form" onSubmit={saveEdit}><header><span><Settings2 size={15} /></span><div><strong>Edit /{editingWorkflow.skillSlug}</strong><small>Changing the name also updates its slash command.</small></div></header><label className="field"><span>Skill name</span><input value={editName} onChange={(event) => setEditName(event.target.value)} required autoFocus /></label><label className="field"><span>Starting web page</span><input value={editUrl} onChange={(event) => setEditUrl(event.target.value)} required /></label><div className="form-actions"><button type="button" className="button-secondary" onClick={() => setEditingWorkflow(null)}>Cancel</button><button className="button-primary" disabled={busy}>{busy ? <LoaderCircle className="spinner" size={14} /> : <Check size={14} />} Save skill</button></div></form>}
  </div>;
}
...
export function App() {
  const urlParams = new URLSearchParams(window.location.search), googleResult = urlParams.get("google"), connectorResult = urlParams.get("status"), connectorName = urlParams.get("connector");
  const callbackNotice = googleResult === "connected" ? "Google is connected" : googleResult === "attention" ? "Google needs one more step" : connectorName && connectorResult === "connected" ? `${connectorName === "slack" ? "Slack" : "Notion"} is connected` : connectorName && connectorResult === "attention" ? `${connectorName === "slack" ? "Slack" : "Notion"} needs another try` : null;
  const [state, setState] = useState<AppState | null>(null), [activeThreadId, setActiveThreadId] = useState("team-room"), [selectedBotIds, setSelectedBotIds] = useState<string[]>([]), [provider, setProvider] = useState<ProviderStatus | null>(null), [connectors, setConnectors] = useState<ConnectorStatus | null>(null), [panel, setPanelState] = useState<Panel>(panelFromLocation), [creatingBot, setCreatingBot] = useState(false), [sidebarOpen, setSidebarOpen] = useState(false), [sending, setSending] = useState(false), [toast, setToast] = useState<string | null>(callbackNotice), [authRequired, setAuthRequired] = useState(false), [connection, setConnection] = useState<ConnectionState>(navigator.onLine ? "reconnecting" : "offline"), [installPrompt, setInstallPrompt] = useState<InstallPrompt | null>(null);
  const messagesEnd = useRef<HTMLDivElement>(null), previousRuns = useRef<Record<string, string>>({}), previousAlerts = useRef<Set<string> | null>(null);
  const setPanel = useCallback((next: Panel) => {
    setPanelState(next);
    const url = new URL(window.location.href);
    if (next) url.searchParams.set("panel", next); else url.searchParams.delete("panel");
    url.searchParams.delete("google"); url.searchParams.delete("connector"); url.searchParams.delete("status");
  ...
  useEffect(() => {
    ...
    for (const run of state.runs) {
      ...
      if (!run.parentRunId && prior && prior !== run.status && ["completed", "awaiting_approval", "failed"].includes(run.status) && document.hidden && Notification.permission === "granted") {
    ...
    const currentAlerts = new Set(state.automationAlerts.map((alert) => alert.id));
    for (const alert of state.automationAlerts) {
      if (previousAlerts.current && !previousAlerts.current.has(alert.id) && document.hidden && Notification.permission === "granted") new Notification("OpenBot automation", { body: alert.message, icon: "/icon.svg" });
    }
  ...
  const activeRuns = state?.runs.filter((run) => (!run.parentRunId || run.status === "awaiting_approval") && !["completed", "cancelled"].includes(run.status)).reverse() || [];
  ...
  return <div className="app-shell">
    ...
    {panel === "teach" && activeBot && <Sheet title={`Teach ${activeBot.name}`} subtitle="Turn a demonstration into a reusable skill" onClose={() => setPanel(null)}><TeachPanel bot={activeBot} onNotice={setToast} onUse={async (workflow) => { setPanel(null); selectThread(activeBot.threadId); await api("/api/messages", { method: "POST", body: JSON.stringify({ threadId: activeBot.threadId, body: `/${workflow.skillSlug}`, targetBotIds: [activeBot.id], attachmentIds: [] }) }); await loadState(activeBot.threadId, true); setToast(`${workflow.name} started`); }} /></Sheet>}
    {panel === "routines" && <Sheet title="Automations" subtitle="Reliable work that wakes up at the right moment" onClose={() => setPanel(null)}><RoutinesPanel routines={state.routines} events={state.automationEvents} alerts={state.automationAlerts} bots={state.bots} onCreate={async (input) => { const result = await api<RoutineSaveResult>("/api/routines", { method: "POST", body: JSON.stringify(input) }); await loadState(activeThreadId, true); setToast("Automation created"); return result; }} onUpdate={async (routine, input) => { const result = await api<RoutineSaveResult>(`/api/routines/${routine.id}`, { method: "PATCH", body: JSON.stringify(input) }); await loadState(activeThreadId, true); setToast("Automation updated"); return result; }} onToggle={async (routine) => { await mutate(() => api(`/api/routines/${routine.id}`, { method: "PATCH", body: JSON.stringify({ enabled: !routine.enabled }) })); }} onDelete={async (routine) => { await mutate(() => api(`/api/routines/${routine.id}`, { method: "DELETE" }), "Automation deleted — past results were kept"); }} onRun={async (routine) => { await mutate(() => api(`/api/routines/${routine.id}/run`, { method: "POST", body: JSON.stringify({ confirmed: true }) }), "Test run started"); }} onReplay={async (event) => { await mutate(() => api(`/api/automation-events/${event.id}/replay`, { method: "POST" }), "Event queued again"); }} onRotateSecret={async (routine) => { const result = await api<RoutineSaveResult>(`/api/routines/${routine.id}/rotate-secret`, { method: "POST" }); await loadState(activeThreadId, true); setToast("New signing secret created"); return result; }} onResolveAlert={async (alert) => { await mutate(() => api(`/api/automation-alerts/${alert.id}/resolve`, { method: "POST" }), "Alert cleared"); }} onOpenResult={(routine) => { setPanel(null); selectThread(routine.threadId); }} /></Sheet>}
