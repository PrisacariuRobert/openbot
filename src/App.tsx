...
type RoutineInput = { name: string; botId: string; threadId: string; prompt: string; intervalMinutes: number; enabled: boolean };

function RoutinesPanel({ routines, bots, onCreate, onUpdate, onToggle, onDelete, onRun, onOpenResult }: { routines: Routine[]; bots: Bot[]; onCreate: (input: RoutineInput) => Promise<void>; onUpdate: (routine: Routine, input: RoutineInput) => Promise<void>; onToggle: (routine: Routine) => Promise<void>; onDelete: (routine: Routine) => Promise<void>; onRun: (routine: Routine) => Promise<void>; onOpenResult: (routine: Routine) => void }) {
  ...
  const [editing, setEditing] = useState<Routine | null>(null), [openHistory, setOpenHistory] = useState<string | null>(null), [history, setHistory] = useState<Record<string, Run[]>>({}), [historyBusy, setHistoryBusy] = useState<string | null>(null);
  ...
  const reset = () => { setName(""); setPrompt(""); setSchedule("1440"); setCustomAmount(2); setCustomUnit("hours"); setEnabled(true); setEditing(null); setCreating(false); };
  const startCreate = () => { reset(); setCreating(true); };
  const startEdit = (routine: Routine) => {
    setEditing(routine); setCreating(true); setName(routine.name); setPrompt(routine.prompt); setBotId(routine.botId); setEnabled(routine.enabled);
  ...
  const showHistory = async (routine: Routine, force = false) => {
    if (!force && openHistory === routine.id) return setOpenHistory(null);
    setOpenHistory(routine.id); setHistoryBusy(routine.id);
    try { setHistory({ ...history, [routine.id]: await api<Run[]>(`/api/routines/${routine.id}/runs`) }); }
  ...
  const submit = async (event: FormEvent) => {
    ...
    setSaving(true);
    try {
      const input = { name, prompt, botId: selectedBot.id, threadId: selectedBot.threadId, intervalMinutes, enabled };
      if (editing) await onUpdate(editing, input); else await onCreate(input);
      reset();
  ...
  return <div className="routines-view">
    ...
    {routines.length > 0 && <div className="routine-list">{routines.map((routine) => { const bot = bots.find((item) => item.id === routine.botId)!; const runs = history[routine.id] || []; return <article key={routine.id} className={`routine-card ${routine.enabled ? "is-on" : ""}`}><div className="routine-row"><Mascot bot={bot} size="small" /><div><strong>{routine.name}</strong><span>{routine.botName} · {routineScheduleLabel(routine.intervalMinutes)}</span><small>{routine.enabled ? lastStatus(routine) : "Paused"}{routine.nextRunAt && routine.enabled ? ` · next ${relativeTime(routine.nextRunAt) === "Now" ? "soon" : `in ${routineStartsInLabel(Math.max(5, Math.round((new Date(routine.nextRunAt).getTime() - Date.now()) / 60_000))).replace(/^Starts /, "")}`}` : ""}</small></div><button className="routine-play" onClick={() => void onRun(routine)} title="Run once now" aria-label={`Run ${routine.name} now`}><Play size={13} /></button><button className={`toggle ${routine.enabled ? "on" : ""}`} onClick={() => void onToggle(routine)} aria-label={`${routine.enabled ? "Pause" : "Start"} ${routine.name}`}><span /></button></div><div className="routine-actions"><button onClick={() => startEdit(routine)}><Settings2 size={13} /> Edit</button><button onClick={() => void showHistory(routine)}><Clock3 size={13} /> {openHistory === routine.id ? "Hide history" : `History${routine.runCount ? ` · ${routine.runCount}` : ""}`}</button><button className="danger" onClick={() => { if (window.confirm(`Delete “${routine.name}”? Its past conversation results will stay available.`)) void onDelete(routine); }}><Trash2 size={13} /> Delete</button></div>{openHistory === routine.id && <div className="routine-history">{historyBusy === routine.id ? <div className="routine-history-empty"><LoaderCircle className="spinner" size={16} /> Loading past runs…</div> : runs.length ? runs.map((run) => <div key={run.id} className={`routine-history-row history-${run.status}`}><span className="history-state">{run.status === "completed" ? <Check size={12} /> : run.status === "failed" ? "!" : <LoaderCircle className={run.status === "running" ? "spinner" : ""} size={12} />}</span><span><strong>{run.status === "completed" ? "Finished" : run.status === "failed" ? "Needs another try" : run.status === "cancelled" ? "Stopped" : "In progress"}</strong><small>{run.summary || run.error || run.task.verificationSummary || "The result will appear in the conversation."}</small></span><time>{relativeTime(run.finishedAt || run.startedAt || run.progressAt)}</time>{["failed", "cancelled"].includes(run.status) && <button onClick={() => void onRun(routine).then(() => showHistory(routine, true))}><RefreshCw size={12} /> Retry</button>}<button onClick={() => onOpenResult(routine)}><ExternalLink size={12} /> Open</button></div>) : <div className="routine-history-empty"><Clock3 size={16} /> No runs yet. Use the play button to test it now.</div>}</div>}</article>; })}</div>}
    {creating ? <form className="routine-form" onSubmit={submit}>
      <header className="routine-form-head"><span>{editing ? <Settings2 size={17} /> : <Sparkles size={17} />}</span><div><strong>{editing ? "Edit routine" : "New routine"}</strong><small>{editing ? "Change the job, owner or timing whenever you need." : "Give your teammate a job and a rhythm."}</small></div></header>
      ...
      <div className="form-actions"><button type="button" className="button-secondary" onClick={reset}>Cancel</button><button className="button-primary" disabled={!intervalValid || saving}>{saving ? <><LoaderCircle className="spinner" size={15} /> Saving…</> : editing ? "Save changes" : "Create routine"}</button></div>
    </form> : <button className="add-routine" onClick={startCreate}><Plus size={17} /> Add a routine</button>}
  </div>;
}
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    {panel === "routines" && <Sheet title="Routines" subtitle="Small jobs that remember themselves" onClose={() => setPanel(null)}><RoutinesPanel routines={state.routines} bots={state.bots} onCreate={async (input) => { await mutate(() => api("/api/routines", { method: "POST", body: JSON.stringify(input) }), "Routine created"); }} onUpdate={async (routine, input) => { await mutate(() => api(`/api/routines/${routine.id}`, { method: "PATCH", body: JSON.stringify(input) }), "Routine updated"); }} onToggle={async (routine) => { await mutate(() => api(`/api/routines/${routine.id}`, { method: "PATCH", body: JSON.stringify({ enabled: !routine.enabled }) })); }} onDelete={async (routine) => { await mutate(() => api(`/api/routines/${routine.id}`, { method: "DELETE" }), "Routine deleted — past results were kept"); }} onRun={async (routine) => { await mutate(() => api(`/api/routines/${routine.id}/run`, { method: "POST" }), "Run started"); }} onOpenResult={(routine) => { setPanel(null); selectThread(routine.threadId); }} /></Sheet>}
