...
import type {
  ...
  GitHubIssueSummary, GitHubNotificationSummary, GoogleConnectorService, MascotKind, Message, ProviderCatalogEntry, ProviderKind, ProviderLoginAttempt, ProviderStatus, Routine, Run,
...
function ConnectorPanel({ status, bots, onRefresh, onNotice, onStartWorkflow }: { status: ConnectorStatus | null; bots: Bot[]; onRefresh: () => Promise<void>; onNotice: (message: string) => void; onStartWorkflow: (prompt: string) => Promise<void> }) {
  ...
  return <div className="connector-panel">
    <div className="connector-hero">
      <div className="connector-hero-copy"><span className="control-kicker"><Sparkles size={13} /> A calmer way to get things done</span><h3>Connect once. Give each teammate only what they need.</h3><p>Google Workspace and GitHub bring live work into the studio—while every external write still waits for your okay.</p></div>
...
function BotPanel({ bot, provider, apps = [], onSave, onOpenTeach }: { bot: Bot; provider: ProviderStatus | null; apps?: ConnectorStatus["access"]; onSave: (id: string, patch: Partial<Bot>) => Promise<void>; onOpenTeach: () => void }) {
  ...
  const gmail = apps.find((access) => access.botId === bot.id && access.service === "gmail"), drive = apps.find((access) => access.botId === bot.id && access.service === "google-drive"), calendar = apps.find((access) => access.botId === bot.id && access.service === "google-calendar"), github = apps.find((access) => access.botId === bot.id && access.service === "github");
